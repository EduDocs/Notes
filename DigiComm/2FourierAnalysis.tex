...
\begin{equation*}
\hat{x}(f) = \sum_{k=-\infty}^{\infty} s_k e^{j 2 \pi \frac{k}{F} f}
\mathrm{rect} \left( \frac{f}{F} \right)
\end{equation*}
where the coefficients $s_k$ are equal to
\begin{equation*}
s_k
...
= \mathcal{F}^{-1} \left[ \sum_{k=-\infty}^{\infty} s_k e^{j 2 \pi \frac{k}{F} f}
\mathrm{rect} \left( \frac{f}{F} \right) \right] \\
&= \sum_{k=-\infty}^{\infty} s_k \mathcal{F}^{-1} \left[ e^{j 2 \pi \frac{k}{F} f}
\mathrm{rect} \left( \frac{f}{F} \right) \right] \\
...
\end{split}
\end{equation*}
We can obtain the values of $s_k$ explicitly by exploiting the properties of the $\mathrm{sinc} (\cdot)$ function, with
\begin{equation*}
x(nT)
= \sum_{k=-\infty}^{\infty}
