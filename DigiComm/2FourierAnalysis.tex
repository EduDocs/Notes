...
Suppose that $s(t)$ is a function that is nonzero only for $-T/2 \leq t \leq T/2$, and is square integrable
\begin{equation*}
...
\hat{s}_k e^{j 2 \pi \frac{k}{T} t}, & |t| \leq \frac{T}{2} \\
0, & \text{otherwise} \end{cases}
\end{equation}
where the Fourier series coefficients $\hat{s}_k$ are given by
\begin{equation*}
\hat{s}_k = \frac{1}{T} \int_{-\frac{T}{2}}^{\frac{T}{2}}
s(t) e^{-j 2 \pi \frac{k}{T} t} dt .
\end{equation*}
We can use the standard rectangular function $\mathrm{rect}(t)$, defined by
...
\begin{equation*}
\theta_k (t) = e^{j 2 \pi \frac{k}{T} t} \mathrm{rect} \left(\frac{t}{T} \right) .
\end{equation*}
...
&= \int_{-\frac{T}{2}}^{\frac{T}{2}} e^{j 2 \pi \frac{k}{T} t}
e^{- j 2 \pi \frac{n}{T} t} dt \\
&= \int_{-\frac{T}{2}}^{\frac{T}{2}} e^{j 2 \pi \frac{(k-n)}{T} t} dt
= 0
\end{split}
\end{equation*}
for all $k \neq n$.
An interesting and important aspect of Fourier series is that time-limited functions can be characterized using a sequence of coefficients.
...
= \int_{\mathbb{R}} x(t) e^{-j 2 \pi f t} dt .
\end{equation}
...
x(t) = \int_{\mathbb{R}} \hat{x}(f) e^{j 2 \pi f t} df .
\end{equation}
We sometimes denote the inverse Fourier transform of $\hat{x}(f)$ as $\mathcal{F}^{-1} [\hat{x}(f)]$.
It is interesting to point out the duality between the Fourier transform and its inverse.
This is emphasized by the striking similarity between \eqref{equation:FourierTransform} and \eqref{equation:InverseFourierTransform}.

\begin{example}[Rectangular Pulse]
The rectangular pulse $\mathrm{rect} (t)$, defined in \eqref{equation:RectangularFunction}, can be used to constrain various signals.
...
&= \int_{\mathbb{R}} \mathrm{rect} (t) e^{- j 2 \pi f t} dt
...
\begin{split}
x(t) &= \int_{\mathbb{R}} \hat{x}(f) e^{j 2 \pi f t} df
= \int_{\mathbb{R}} \left[ \int_{\mathbb{R}} x(\tau) e^{-j 2 \pi f \tau} d\tau \right] e^{j 2 \pi f t} df \\
&= \int_\tau \left[ \int_{\mathbb{R}} e^{j 2 \pi f (t - \tau)} df \right] x(\tau) d\tau ,
\end{split}
\end{equation}
...
\begin{equation*}
\delta (t) = \int_{\mathbb{R}} e^{j 2 \pi f t} df
\end{equation*}
...
x(t) = \sum_{k = -\infty}^{\infty} \hat{s}_k e^{j 2 \pi k f_0 t} .
\end{equation}
...
x(t) = \sum_{k = -\infty}^{\infty} \delta(t - kT)
= \frac{1}{T} \sum_{k = -\infty}^{\infty} e^{j 2 \pi \frac{k}{T} t} .
\end{equation}
...
A signal $x(t)$ is bandwidth-limited to $W$ if it can be obtained as the inverse Fourier tranform of a function $\hat{x}(f)$, where $\hat{x}(f)$ is identically zero for all $|f| > W$.
...
If the signal $x(t)$ is bandwidth-limited to $W$, then this signal can be reconstructed from its samples so long as they are taken at periodic intervals of $T \leq \frac{1}{2W}$.
A formal version of the sampling theorem appears below.

\begin{theorem}[Sampling Theorem] \label{theorem:SamplingTheorem}
Suppose that $x(t)$ is a bandwidth-limited signal with bandwidth $W$.
...
Indeed, if $T \leq \frac{1}{2W}$ then
...
For convenience, we define $F = \frac{1}{T}$ and we emphasize that $W \geq \frac{1}{2T} = \frac{F}{2}$.
Thus, $\hat{x}(f) = 0$ whenever $|f| > \frac{F}{2}$.
We can use the theory of Fourier series introduced in Section~\ref{section:FourierSeries} to express $\hat{x}(f)$ as
\begin{equation*}
\hat{x}(f) = \sum_{k=-\infty}^{\infty} \hat{s}_k e^{j 2 \pi \frac{k}{F} f}
\mathrm{rect} \left( \frac{f}{F} \right)
\end{equation*}
where the coefficients $\hat{s}_k$ are equal to
\begin{equation*}
\hat{s}_k
= \frac{1}{F} \int_{-\frac{F}{2}}^{\frac{F}{2}}
\hat{x}(f) e^{- j 2 \pi \frac{k}{F} f} df .
\end{equation*}
Special care must be taken when reading these results;
we are applying Fourier series analysis to a function in the frequency domain.

We can write $x(t)$ in terms of basis elements,
\begin{equation*}
\begin{split}
x(t) &= \mathcal{F}^{-1} \left[ \hat{x}(f) \right]
= \mathcal{F}^{-1} \left[ \sum_{k=-\infty}^{\infty} \hat{s}_k e^{j 2 \pi \frac{k}{F} f}
\mathrm{rect} \left( \frac{f}{F} \right) \right] \\
&= \sum_{k=-\infty}^{\infty} \hat{s}_k \mathcal{F}^{-1} \left[ e^{j 2 \pi \frac{k}{F} f}
\mathrm{rect} \left( \frac{f}{F} \right) \right] \\
%&= \sum_{k=-\infty}^{\infty} \hat{s}_k
%\int_{-\frac{F}{2}}^{\frac{F}{2}} e^{j 2 \pi \frac{k}{F} f} e^{j 2 \pi f t} df \\
%&= \sum_{k=-\infty}^{\infty} \hat{s}_k
%\int_{-\frac{F}{2}}^{\frac{F}{2}} e^{j 2 \pi \left( t + \frac{k}{F} \right) f} df \\
%&= \sum_{k=-\infty}^{\infty} \hat{s}_k
%\frac{e^{j \pi \left( t + \frac{k}{F} \right) F} - e^{-j \pi \left( t + \frac{k}{F} \right) F}}{j 2 \pi \left( t + \frac{k}{F} \right)} \\
%&= \sum_{k=-\infty}^{\infty} \hat{s}_k
%\frac{\sin \left( \pi \left( t + \frac{k}{F} \right) F \right)}{\pi \left( t + \frac{k}{F} \right)} \\
&= \sum_{k=-\infty}^{\infty} \hat{s}_k F \mathrm{sinc} \left( F t + k \right) \\
&= \sum_{k=-\infty}^{\infty} \frac{\hat{s}_k}{T} \mathrm{sinc} \left( \frac{t}{T} + k \right) .
\end{split}
\end{equation*}
We can obtain the values of $\hat{s}_k$ explicitly by exploiting the properties of the $\mathrm{sinc} (\cdot)$ function, with
\begin{equation*}
x(nT)
= \sum_{k=-\infty}^{\infty}
\frac{\hat{s}_k}{T} \mathrm{sinc} \left( \frac{nT}{T} + k \right)
= \sum_{k=-\infty}^{\infty}
\frac{\hat{s}_k}{T} \mathrm{sinc} ( n + k ) = \frac{\hat{s}_{-n}}{T} .
\end{equation*}
Thus, we have $s_{k} = T x(-kT)$ and formula \eqref{equation:SamplingReconstructionFormula} follows.
The sampling rate corresponding to $T = \frac{1}{2W}$ is the minimum sampling rate at which no aliasing occurs.
...
When the sampling rate exceeds twise the bandwidth of a signal, the original signal can be reconstituted from its sampled values.}
...
&= \int_{\mathbb{R}} R_X (\tau) e^{-j2 \pi f\tau} d\tau
= \mathcal{F} [ R_X (\tau) ] .
\end{split}
\end{equation*}
...
Let $\tilde{X}(t)$ be the estimate for $X(t)$ built from the sampled values $\{ X(nT) : n \in \mathbb{Z} \}$; that is,
\begin{equation*}
...
\begin{equation*}
\begin{split}
&\sum_{n=-\infty}^{\infty} \sum_{p=-\infty}^{\infty} R_X(pT)
\mathrm{sinc}(2 W (t - pT - nT)) \mathrm{sinc}(2 W (t - nT)) \\
&\sum_{n=-\infty}^{\infty} \left( \sum_{p=-\infty}^{\infty} R_X(pT)
\mathrm{sinc}(2 W (t - pT - nT)) \right) \mathrm{sinc}(2 W (t - nT)) \\
...
Its Fourier transform $\hat{y}(f)$ is non-zero only for frequencies contained in a small neighborhood of carrier frequency $f_0$.
That is, $\hat{y}(f) = 0$ for all frequencies such that $|f - f_0| \geq W$.
...
\begin{equation*}
\hat{y}(f) = \frac{1}{2} \hat{x}(f+f_0) + \frac{1}{2} \hat{x}(f - f_0) .
\end{equation*}
Our strategy is to first eliminate $\frac{1}{2} \hat{x}(f + f_0)$ from $\hat{y}(f)$, and then to scale and shift $\frac{1}{2} \hat{x}(f + f_0)$ back to the origin.
...
\end{equation*}
And, by construction, we obtain $\hat{x}(f - f_0) = 2 u(f) \hat{y}(f)$.
