...
&= \int_\tau \left[ \int_{\mathbb{R}} e^{2 \pi i f (t - \tau)} df \right] x(\tau) d\tau ,
\end{split}
\end{equation}
...
\hat{x}_{\mathrm{s}}(f) &= \hat{x}(f) \star \mathcal{F} \left[ \sum_{n=-\infty}^{\infty} \delta(t-nT) \right] \\
...
&= \mathcal{S}_X(f) \left| \hat{h}(f) \right|^2 .
\end{split}
\end{equation*}
...
\end{equation*}
And, by construction, we obtain $\hat{x}(f - f_0) = 2 \mathrm{step}(f) \hat{y}(f)$.
We can therefore recover the original lowpass signal $x(t)$ using the frequency-shift property of the Fourier transform,
\begin{equation*}
x(t)
= \left[ y(t) \star \left( \delta (t) + \frac{i}{\pi t} \right) \right]
e^{- 2 \pi i f_0 t}
= \left[ y(t) + i \left( y(t) \star \frac{1}{\pi t} \right) \right] e^{- 2 \pi i f_0 t} .
\end{equation*}
