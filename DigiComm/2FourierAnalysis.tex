...
\end{theorem}
\begin{proof}
Let $x_{s}(t)$ denote the result of sampling $x(t)$ by impulses at times $\{ nT : n \in \mathbb{Z} \}$,
\begin{equation*}
...
\begin{equation*}
\begin{split}
\hat{x}_s(f) &= \hat{x}(f) \star \mathcal{F} \left[ \sum_{n=-\infty}^{\infty} \delta(t-nT) \right] \\
&= \hat{x}(f) \star \frac{1}{T} \sum_{n=-\infty}^{\infty} \delta \left( f-\frac{n}{T} \right) \\
...
\begin{equation*}
\hat{h}(f) = T \mathrm{rect} \left( \frac{f}{2W} \right)
= \begin{cases} T, & |f| < W \\
0, & |f| > W \end{cases}
\end{equation*}
...
\begin{equation*}
\hat{x}_s(f) \hat{h}(f)
= \frac{1}{T} \left[ \sum_{n=-\infty}^{\infty} \hat{x} \left( f - \frac{n}{T} \right) \right]
T \mathrm{rect} \left( \frac{f}{2W} \right)
= \hat{x}(f) .
\end{equation*}
The output of the ideal lowpass filter $\hat{h}(f)$ is the original signal $x(t)$, and perfect reconstruction is achieved.
