...
Although the search space for uniform quantizers is much smaller than the set of all possilbe quatizers, finding an optimal uniform quantizer remains a strenuous task in general.
The resulting minimazation problem need not have a closed-form solution, in contrast to Example~\ref{example:OptimalUniformQuantizer}.
...
Suppose that the quantization points $\{ q_i \}$ are fixed.
The optimal quantizer given these points is the function $Q^* : \mathbb{R} \mapsto \mathcal{Q}$ that minimizes the MSE,
\begin{equation*}
\begin{split}
...
\end{equation*}
In particular, an optimal $\ell$-level scalar quantizer always has $\ell$ decision regions, each being an interval around a quantization point.
...
\end{equation*}
The value of the MSE can be minimize by selecting the quantization points $\{ q_i \}$ appropriately.
Note that each quantization point can be optimized individually, as the integral has a nice additive structure.
...
\begin{equation*}
\frac{\partial}{\partial q_i} \int_{b_{i-1}}^{b_i} (\xi - q_i)^2 f_x(\xi) d\xi
...
\end{equation*}
In other words, once the decision intervals are picked, the optimal quantization point $q_i^*$ is equal to the conditional expectation of $x$ given that $x$ falls inside interval~$i$.


\begin{equation} \label{equation:QuantizerMSE}
\begin{split}
\text{MSE} &= \int_{-\infty}^{\frac{q_1 + q_2}{2}} (\xi - q_1)^2 f_x(\xi) d\xi
+ \sum_{i=2}^{{\ell}-1}
\int_{\frac{q_{i-1} + q_i}{2}}^{\frac{q_i + q_{i+1}}{2}}
(\xi - q_i)^2 f_x(\xi) d\xi \\
&+ \int_{\frac{q_{\ell-1} + q_{\ell}}{2}}^{\infty} (\xi - q_{\ell})^2 f_x(\xi) d\xi .
\end{split}
\end{equation}
