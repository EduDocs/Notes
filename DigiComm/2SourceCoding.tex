...
%, leading to simple interpretations.
...
% it serves as a measure of information in that
...
To model these sources, we use the mathematical concept of a \defn{random process}{discrete-time} \textbf{random process}.
...
In this section, we introduce a simple random process with memory known as a \defn{random process}{Markov chain}.
...
If the conditional probability is time-invariant (i.e., it doesn't depend on $n$), then the Markov chain is called \defn{random process}{stationary}.
