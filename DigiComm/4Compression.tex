...
\end{equation*}
The optimal value for $\ell(x)$, which we denote by $\ell^*(x)$, must therefore satisfy $2^{- \ell^*(x)} = p_X(x) / (\lambda \ln (2))$.
Computing the derivative with respect to $\lambda$ yields
\begin{equation*}
\sum_{x \in \mathcal{X}} 2^{- \ell^*(x)} = 1 ,
\end{equation*}
...
Suppose that we wish to encode outputs from discrete memoryless source $(\mathcal{X}, p_X)$, and let $c^* : \mathcal{X} \mapsto \mathcal{C}$ be an optimal prefix-code for this source.
If $p_X(x_1) > p_X(x_2)$, then $\ell_{c^*}(x_1) \leq \ell_{c^*}(x_2)$.
...
\epsfig{file=Figures/huffman1,width=4in}
\end{psfrags}
...
\psfrag{x3}[c]{$x_3$}
\psfrag{x4}[c]{$x_4$}
\epsfig{file=Figures/huffman2,width=4in}
\end{psfrags}
