...
The entropy of $X$, denoted $H[X]$, is given by
...
It is important to recognize that $H[X]$ is computed based on the probability mass function $p_X(\cdot)$, it is not a function of the random variable $X$ itself.
...
\end{equation*}
Then, by applying Theorem~\ref{theorem:EntropyBoundsPrefixCodes}, we get that an optimal perfix code, which operates on $\mathcal{X}^n$, yields
\begin{equation*}
...
\end{equation*}
Thus, the expected number of bits per symbol produced by a source can be made arbitrarily close to $H[X]$ by jointly encoding strings of symbols.
...
\begin{equation*}
H[X] = - \frac{3}{4} \log_2 \left( \frac{3}{4} \right)
