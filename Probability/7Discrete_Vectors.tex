\chapter{Discrete Random Vectors}
...
We center our exposition of random vectors around the a simple case, pairs of random variables.
...
To further distinguish between the joint PMF of $X$ and $Y$ and the individual PMFs $p_X (\cdot)$ and $p_Y (\cdot)$, we occasionally refer to the latter as \emph{marginal probability mass functions}. \index{Probability mass function (PMF)} \index{Marginal probability mass function}
...
The joint PMF of $X$ and $Y$ is specified below,
\begin{center}
...
We can form a random variable $U$ that describes the sum of the two dice, $U = X + Y$.
\end{example}

The definition of the expectation operator can also be extended to multiple random variables.
...
Let $Y = \IndicatorFcn_S (\cdot)$, the indicator function of $S$, with
\begin{equation*}
\IndicatorFcn_S (\omega) = \begin{cases} 1, & \omega \in S\\
0, & \omega \notin S . \end{cases}
\end{equation*}
Then $p_{X|S} (x) = p_{X|Y} (x | 1)$.
In some sense, conditioning on an event is a special case of a conditional probability mass function.
...
However, if packet transmission fails $n$ consecutive times, then the data is dropped from the queue altogether, with not further transmission attempt.
...
In this sense, the conditional expectation $\Expect [Y | X]$ is simply an instance of a random variable.
...
Note that, in computing the conditional expectation, we have used the memoryless property of the geometric random variable.
\end{example}
...
Random variables $X$ and $Y$ are independent if and only if the events $\{ X = x \}$ and $\{ Y = y \}$ are independent for every $x \in X(\Omega)$ and every $y \in Y(\Omega)$.
...
Furthermore, it is straightforward to show that $X$ and $Y$, the numbers of dots on the two dice, are independent random variable.
...
Suppose $X$ and $Y$ are two independent random variables that take on integer values, and let $U = X + Y$.
...
G_Y (z) &= e^{\beta (z-1)} .
\end{align*}
As such, the ordinary generating function of $U = X + Y$ becomes
\begin{equation*}
...
The concept of joint distribution can be extended to any number of random variables.
...
When the random variables $X_1, X_2, \ldots, X_n$ are independent, their joint PMF reduces to
\begin{equation*}
...
\begin{equation*}
p_{Y_i} (m) = \frac{1}{m!} \frac{d^m G_{Y_i}}{dz^k} (0) = p (1-p)^m
\quad m = 0, 1, \ldots
\end{equation*}
