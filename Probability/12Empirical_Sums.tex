...
\begin{example}
Again, let $X_1, X_2, \ldots$ be a sequence as describe above, and let $S_n$ be defined according to \eqref{equation:GaussianEmpiricalSum}.
...
We therefore resort to complex analysis and contour integration to get the answer.
...
\end{split}
\end{equation*}
Again, only two residues are contained within the enclosed region.
Because they are simple pole, their values are given by
\begin{align*}
...
We can then conclude that the PDF of $S$ is equal to
\begin{equation*}
...
S_n = \sum_{i=1}^n X_n .
\end{equation*}
Using mathematical induction and the aforementioned fact, it is straightforward to show that $S_n$ is a Cauchy random variable with parameter $n \gamma$.
...
Note that our version of the law of large numbers requires the random variables to have finite second moment, a condition that is clearly violated by the Cauchy distribution.
...
\begin{equation*}
\Lambda_X (s) = \log \left( M_X (s) \right)
= \log \left( \Expect \left[ e^{sX} \right] \right) .
\end{equation*}
...
Next, we consider the log-moment generating function of $S_n / \sqrt{n}$.
...
\begin{equation*}
\begin{split}
\log \left( \Expect \left[ e^{s S_n / \sqrt{n} } \right] \right)
&= \log \left( \Expect \left[ e^{s X_1 / \sqrt{n} }
\cdots e^{s X_n / \sqrt{n} } \right] \right) \\
&= \log \left( M_X \left( \frac{s}{\sqrt{n}} \right)
\cdots M_X \left( \frac{s}{\sqrt{n}} \right) \right) \\
&= n \Lambda_X \left( s n^{-1/2} \right) .
\end{split}
\end{equation*}
To explore the asymptotic behavior of $S_n / \sqrt{n}$, we take the limit of $n \Lambda_X \left( s n^{-1/2} \right)$ as $n \rightarrow \infty$.
...
In other words, $S_n / \sqrt{n}$ converges in distribution to a standard normal random variable.
...
In the last step of the proof, we stated that point-wise convergence of the moment generating functions implies convergence in distribution, a sophisticated result we quote without proof.
