...
A \emph{function} is a special type of relation that assigns exactly one value to each input.\index{Function}
...
The \emph{image} of function is the set of all objects of the form $f(x)$, where $x$ ranges over the elements of $X$,\index{Image}
\begin{equation*}
\{ f(x) \in Y | x \in X \} .
\end{equation*}
...
The function $f$ is \emph{surjective} or \emph{onto} if its image is equal to its codomain.\index{Surjective}\index{Onto}
