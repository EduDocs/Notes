...
Recall that a \emph{random variable} is a real-valued function of the outcome of an experiment. \index{Random variable}
In particular, given a sample space, $X$ is a function from $\Omega$ to $\RealNumbers$.
The \emph{cumulative distribution function (CDF)} of random variable $X$ is defined using the probability of the event $\{X \leq x \}$, \index{Cumulative distribution function}
\begin{equation*}
...
We denote the derivative by $f_X (x)$ and call it the \emph{probability density function (PDF)} of $X$. \index{Probability density function (PDF)}
...
For $x_1 < x_2$, we can combine the definition of $f_X(x)$ and \eqref{equation:IntervalCDF} to obtain
\begin{equation*}
...
Our intuition about continuous random variables can be developed by looking at examples.
...
\end{center}
\caption{The PDF of a uniform random variable appears above for intervals $[0,1]$, $[0,2]$ and $[0,4]$.}
\end{figure}
...
\end{center}
\caption{The PDF of a Gaussian random variable appears above for parameters $m = 0$ and $\sigma^2 \in \{ 1, 2, 4 \}$.}
\end{figure}

A Gaussian random variable whose distribution has parameters $m = 0$ and $\sigma = 1$ is call a \emph{normal random variable}, a name that points to its great popularity. \index{Normal random variable}
...
The normal random variable is so frequent in applied mathematics and engineering that variations of its CDF possess their own names.
...
The \emph{exponential random variable} is also a very important continuous random variable. \index{Exponential random variable}
...
\end{center}
\caption{The PDF of an exponential random variable appears above for $\lambda \in \{0.5, 1, 2 \}$.}
\end{figure}
...
The \emph{Rayleigh distribution} arises primarily when two zero-mean random variables have Gaussian distribution, are uncorrelated and possess equal variance. \index{Rayleigh random variable}
...
\end{center}
\caption{The PDF of a Rayleigh random variable appears above for $\sigma^2 \in \{ 1, 2, 4 \}$.}
\end{figure}
...
\end{center}
\caption{The PDF of a Laplace random variable appears above for $b \in \{0.5, 1, 2 \}$.}
\end{figure}
...
\end{center}
\caption{The PDF of a Cauchy random variable appears above for $\gamma^2 \in \{ 1, 2, 4 \}$.}
\end{figure}
