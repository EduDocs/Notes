...
The \emph{Cauchy distribution} is considered a heavy-tail distribution because its tails are not exponentially bounded. \index{Cauchy random variable}
The PDF of a Cauchy random variable is given by
\begin{equation*}
f_X (x) = \frac{ \gamma }{\pi \left( \gamma^2 + x^2 \right)} \quad - \infty < x < \infty.
\end{equation*}
...
\end{center}
\caption{NEED TO REGENERATE GRAPH B/C OF TYPO IN PDF.
Cauchy distributions are categorized as heavy-tail distributions because of their very slow decay.
