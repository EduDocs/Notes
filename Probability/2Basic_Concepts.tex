...
\psfrag{E}[l]{event}
\psfrag{O}[r]{outcome}
\includegraphics[height=4.13cm]{Figures/2Chapter/samplespace}
\end{psfrags}
...
\caption{This set cannot be a sample space as the elements are not mutually exclusive.}
\end{center}
\end{figure}
...
Moreover, this space is collectively exhaustive since every integer between one and six is either odd or even.
...
\end{center}
\caption{A possible sample space for the rolling of a die is composed of two sets, the odd numbers and the even numbers between one and six.}
\end{figure}
\end{example}
...
We can use Proposition~\ref{proposition:ProbabilityUnion} to derive a bound on the probability of unions.
This result, which is sometimes called the \emph{Boole inequality}, states that the probability of at least one event occurring is no greater than the sum of the probability of the individual events.
\index{Boole inequality}
...
\end{split}
\end{equation*}
Thus, by the principle of mathematical induction, \eqref{equation:UnionBound} is valid for all integers $n > 0$.
\end{proof}
...
Suppose that an element is chosen at random from this interval, with all elements being equally likely.
...
For instance, $\Pr \left( \left( 0, 0.25 \right) \right) = \Pr \left( \left( 0.75, 1 \right) \right)$.
...
\end{psfrags}
\caption{If the outcome of an experiment is uniformly distributed over $[0,1]$, then two subintervals of equal length have the same probabilities.}
\end{center}
\end{figure}

In an extension of the previous observation, we take the probability of an open interval $(a, b)$ where $0 \leq a < b \leq 1$ by
