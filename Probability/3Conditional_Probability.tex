...
\begin{example}
Let $A,B$ be events (i.e., subsets) in the sample space $\Omega$ associated with a particular random experiment and assume that $\Pr(B)>0$.
To gain intuition for more general conditional probabilities, consider repeating the experiment $N$ times and tallying the results relative to $A,B$.
Let $N_{AB}$ be the number of times that both $A$ and $B$ occurred, $N_{A\overline{B}}$ be the number of times that only $A$ occurred,  $N_{\overline{A}B}$ be the number of times that only $B$ occurred, and  $N_{\overline{A}\overline{B}}$ be the number of times that neither occurred.
From this, we see that $A$ occurred exactly $N_A = N_{AB} + N_{A\overline{B}}$ times and $B$ occurred exactly $N_B = N_{AB} + N_{\overline{A}B}$ times.

The frequentist view of probability is based on the fact that, as $N$ becomes large, one can compute event probabilities by taking the ratio of the number of times the event occurred to the number of trials.
This implies that
\begin{equation*}
\Pr (A\cap B) \approx\frac{N_{AB}}{N} \quad \quad
\Pr (B) \approx \frac{N_{B}}{N}.
\end{equation*}
Likewise, the conditional probability that $A$ occurs given knowledge that $B$ occurred can be computed with
\[ \Pr (A | B) \approx \frac{N_{AB}}{N_B} = \frac{N_{AB} / N}{N_B / N} \approx \frac{\Pr (A \cap B)}{\Pr (B)}. \]
As the number of trials approaches infinity, the approximation becomes exact and one obtains the definition of conditional probability.
\end{example}

Having considered intuitive arguments, we now give the mathematical definition of conditional probability.
...
\end{equation}
This formula is known as the \emph{chain rule of probability} and can be verified by expanding each of the conditional probabilities using \eqref{equation:ConditionalProbability},
\begin{equation*}
\Pr \left( \bigcap_{k=1}^n A_k \right)
