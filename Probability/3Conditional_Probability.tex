...
We begin our description of conditional probability with an illustrative example.
...
Let $N_{AB}$ be the number of trials for which $A \cap B$ occurred, $N_{A\overline{B}}$ be the number of times where only $A$ occurred,  $N_{\overline{A}B}$ be the number of times where only $B$ occurred, and  $N_{\overline{A}\overline{B}}$ be the number of trials for which neither took place.
From these definitions, we gather that $A$ was observed exactly $N_A = N_{AB} + N_{A\overline{B}}$ times, and $B$ was seen $N_B = N_{AB} + N_{\overline{A}B}$ times.

The frequentist view of probability is based on the fact that, as $N$ becomes large, one can approximate the probability of an event by taking the ratio of the number of times this event occurred over the total number of trials.
For instance, we can write
\begin{xalignat*}{2}
\Pr (A\cap B) & \approx \frac{N_{AB}}{N} &
\Pr (B) & \approx \frac{N_{B}}{N}.
\end{xalignat*}
Likewise, the conditional probability of $A$ given knowledge that $B$ has occurred can be computed using
\begin{equation*}
\Pr (A | B) \approx \frac{N_{AB}}{N_B}
= \frac{N_{AB} / N}{N_B / N} \approx \frac{\Pr (A \cap B)}{\Pr (B)}.
\end{equation*}
As $N$ approaches infinity, these approximations become exact and unveil the formula for conditional probability.
\end{example}
...
If the first urn is chosen, then the probability of getting a green ball is $5/8$.
...
\end{split}
\end{equation*}
Although the test may appear fairly accurate, the probability that a person with a positive test carries the disease remains small.
\end{example}
...
These events are independent provided that
