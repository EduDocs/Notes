...
In this section, we expand our exposition of continuous random variables to random vectors.
...
More complex scenarios will be considered in the later.
...
Furthermore, $\lim_{x \downarrow -\infty} F_{X,Y} (x,y) 
...
\end{equation}
Hereafter, we refer to a pair of random variables as continuous if the corresponding joint PDF is defined unambiguously through \eqref{equation:JointPDF}.
...
\end{equation*}
Furthermore, for an admissible set $S \subset \RealNumbers^2$, the probability that $(X,Y) \in S$ can be evaluated through the integral form
...
\end{split}
\end{equation}
In particular, if $S$ is the rectangular set
\begin{equation*}
...
\begin{example}
Let $X$ and $Y$ be two independent zero-mean Gaussian random variables with variance $\sigma^2$.
...
Furthermore, if we recognize $R$ as a continuous random variable, we can write its PDF as
\begin{equation*}
f_R (r) = \frac{r}{\sigma^2} e^{- \frac{r^2}{2 \sigma^2} } \quad r \geq 0 .
\end{equation*}
That is, $R$ possesses a Rayleigh distribution with parameter $\sigma^2$.
\end{example}
...
Under proper conditions, it is equally easy to specify the conditional PDF of $X$ given $A$,
\begin{equation*}
...
One case of special interest is the situation where event $A$ is defined in terms of random variable $X$ itself.
...
Essentially, this is equivalent to re-normalizing the PDF of $X$ accounting for the fact that $X \in I$.
...
With care,it is possible and desirable to define the conditional PDF of $X$, conditioned on $Y = y$. \index{Conditional probability density function}
For any $y \in \RealNumbers$ such that $f_Y (y) > 0$, we defined the conditional PDF of $X$ given $Y = y$ as
...
\end{split}
\end{equation*}
Loosely speaking, $f_{X|Y} (x|y) \Delta_x$ represent the probability that $X$ lies close to $x$, given that $Y$ is near $y$.
...
Remember that, in general, conditional probability is only defined for non-vanishing events.
Although we were able to circumvent this issue, care must be taken when dealing with the conditional PDF of \eqref{equation:ConditionalPDF}, as it only provide valuable insight when the pair of random variable $(X, Y)$ is continuous.
...
Deriving the joint PDF of $(Y_1, Y_2)$ can get convoluted, a task forgo.
...
\begin{example}
By far, the most important application of this result relates to the properties of Gaussian vectors.
...
= A \mathbf{X} + \mathbf{b} ,
\end{equation*}
where $A$ is a $2 \times 2$ invertible matrix and $\mathbf{b}$ is a vector.
...
= | A | .
\end{equation*}
It follows that the joint PDF of $(Y_1, Y_2)$ is given by
\begin{equation*}
\begin{split}
&f_{Y_1, Y_2} (y_1, y_2)
...
\end{split}
\end{equation*}
Looking at this equation, we conclude that random variables $Y_1$ and $Y_2$ are jointly Gaussian, as their joint PDF possesses the proper form.
...
= A \Sigma A .
\end{equation*}
These admirable properties generalize to higher dimensions.
An affine transformation of a Gaussian random vector is itself a Gaussian random vector.
Furthemore, to obtain the derived distribution of the latter vector, it suffices to compute its mean and covariance, and substitute the resulting parameters in the general form of the Gaussian PDF.
Collectively, these features underly many contemporary successes of engineering.
\end{example}
...
\end{equation*}
provided that $f_X(x) \neq 0$.
Finally, if $X$ and $Y$ are independent random variables, then the two events $\{ X \in S \}$ and $\{ Y \in T \}$ are independent
\begin{equation*}
\begin{split}
...
Suppose two numbers are independently selected at random from the positive real numbers according to an exponential distribution with parameter $\lambda$. 
...
\begin{example}[Sum of Gaussian Random Variables]
Suppose $X$ and $Y$ are independent Gaussian variables.
