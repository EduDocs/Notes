...
For each $Y\in\mathcal Y$, the set $Z\cap Y'$ contains exactly one ordered pair, say $(Y,y_Y)$.
Define \[ c:\mathcal Y\to\bigcup_{Y\in\mathcal Y}Y\] by $Y \mapsto y_Y$.
Since $(Y,y_Y)\in Y'$, we have $y_Y\in Y$, so $c$ is the desired choice function.
% Define the function
% \begin{equation*}
...
\subsection{The Hausdorff Maximum Principle}
...
\begin{fact}[Hausdorff maximum principle]
...
For example, the inclusion relation $\subseteq$ on a collection of sets is a partial order, whereas proper inclusion $\subsetneq$ is a strict partial order.
