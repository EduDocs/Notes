...
\gpt{Appendix-level feedback: These topics are worthwhile for students who want to understand where existence results in infinite-dimensional linear algebra come from.  However, the label ``Optional Topics'' conflicts with Chapter~3's proof that every vector space has a Hamel basis, which explicitly invokes the maximum principle here.  Either mark that Hamel-basis theorem and proof optional too, or move a concise statement of the Hausdorff maximal principle immediately before its first use and retain this appendix for the equivalences with choice and well-ordering.  For this ECE audience, the latter material is best kept optional unless transfinite arguments are used elsewhere.}
...
\end{lemma}
\begin{proof}
\gpt{The construction below has a type error: $Y'\cap Z$ is a singleton set of ordered pairs, so the displayed map has neither the domain nor the values required by the lemma.  Suggested replacement for lines 48--55: ``For each $Y\in\mathcal Y$, the set $Z\cap Y'$ contains exactly one ordered pair, say $(Y,y_Y)$.  Define $c:\mathcal Y\to\bigcup_{Y\in\mathcal Y}Y$ by $c(Y)=y_Y$.  Since $(Y,y_Y)\in Y'$, we have $y_Y\in Y$, so $c$ is the desired choice function.''}
...
\end{equation*}
\gpt{In the displayed definition above, ``$y\in Y$'' should be ``$y\in X$''; $Y$ is undefined here.}
...
\gpt{Notation: if $\subset$ is used elsewhere for proper inclusion, this sentence contradicts itself.  Suggested replacement: ``For example, inclusion $\subseteq$ is a partial order on a collection of sets, whereas proper inclusion $\subsetneq$ is a strict partial order.''}

\gpt{Terminology: this result is usually called the ``Hausdorff maximal principle.''  Using that name here will help students recognize the standard equivalent form of the axiom of choice and distinguish it from unrelated maximum principles in analysis.}
