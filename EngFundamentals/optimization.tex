...
\gpt{Chapter-level feedback: KKT conditions, duality, Slater's condition, and water-filling are excellent topics for graduate ECE students.  The balance is less effective: more than a quarter of the chapter develops Banach-space derivatives, while unconstrained optimization receives essentially one necessary condition and no algorithmic example.  Consider presenting finite-dimensional gradients, Hessians, gradient descent, and convex first-order conditions first; then place G\^{a}teaux and Fr\'echet derivatives in an advanced section that generalizes the familiar theory.  A short worked gradient-descent example with a convergence bound under Lipschitz gradient/strong convexity would connect the mathematical maturity goals to the algorithms students will use.}
...
Completeness guarantees that Cauchy sequences converge to limits inside the space.

In Chapter~\ref{chap:metric_spaces}, limits are discussed primarily for sequences but Definition~\ref{def:cont_limit} does extend this to cases where points approach other points.
By treating Banach spaces as metric spaces, we arrive at the following induced definition of such limits.

\begin{definition}
  For Banach spaces $X,Y$ with $f\colon X \to Y$ and $x,z \in X$, we define $\lim_{x\to z} f(x)$ as follows.
  For $y\in Y$, we say that $\lim_{x\to z} f(x) = y$ (i.e., the limit as $x \to z$ of $f(x)$ equals $y$) if $f(x_n) \to y$ for all sequences $x_n \in X \setminus \{z\}$ such that $x_n \to z$.
  If there is no such $y\in Y$, then we say the limit does not exist.
  
  The proof of Theorem~\ref{thm:cont_fun_limit} shows that this is equivalent to: for any $\epsilon> 0$, there is a $\delta >0$ such that, for all $x\in X$ satisfying $0<\|x - z\|< \delta$, we have $\| y - f(x) \| < \epsilon$.
\end{definition}
...
If $X$ is a Banach space, then we require that $\delta f (\vecnot{x};\vecnot{h})$ is a continuous linear function of $\vecnot{h}$.
\end{definition}
...
\begin{example}
A function $f\colon \RealNumbers^n \to \RealNumbers^m$ with $f = (f_1,f_2,\ldots,f_m)^T$ is (Fr\'{e}chet) differentiable at $\vecnot{x}_0$ if $f(\vecnot{x}_0 + \vecnot{h})=f(\vecnot{x}_0)+J(\vecnot{x}_0) \vecnot{h} + o(\|\vecnot{h}\|)$ where $J$ is the mapping from $\RealNumbers^n$ to the \defn{optimization}{Jacobian matrix},
...
In that case, all entries of the Jacobian matrix are finite and its operator norm satisfies $\| J(\vecnot{x}) \| < \infty$ because it is finite-dimensional.
A stronger but sufficient condition differentiability is that all partial derivatives exist and are continuous in a neighborhood of $\vecnot x_0$.
...
\gpt{Ordering/assumption issue: the Riesz theorem proved earlier in these notes is explicitly finite-dimensional, whereas this example allows an arbitrary Hilbert space.  Either restrict $X$ to be finite-dimensional here, state the general Riesz representation theorem before this example, or add a forward reference to its later proof.}
...
\begin{theorem} \label{thm:frechet_chain_rule}
...
\begin{proof}
Deferred to Section~\ref{sec:opt_def_proof}.
\end{proof}


\begin{theorem}
\label{theorem:mean_value_inequality}
Let \(X,Y\) be Banach spaces and \(f\colon X\to Y\).
For \(\vecnot{x}_1,\vecnot{x}_2\in X\), let $\vecnot{h}=\vecnot{x}_2-\vecnot{x}_1 \neq \vecnot{0}$
and assume that the G\^{a}teaux differential $\delta f\big((1-s)\vecnot{x}_1+s\vecnot{x}_2;\vecnot{h}\big)$ exists for every \(s\in[0,1]\).
Then, we have
\[
\|f(\vecnot{x}_2)-f(\vecnot{x}_1)\|
\leq
M\|\vecnot{x}_2-\vecnot{x}_1\|,
\]
where
\[
M
...
\sup_{s\in[0,1]}
\left\|
\delta f\big((1-s)\vecnot{x}_1+s\vecnot{x}_2;\vecnot{h}\big)
\right\|
}{
\|\vecnot{x}_2-\vecnot{x}_1\|
}.
\]
\end{theorem}
...
for all $s\in [0,1]$.
Applying Theorem~\ref{theorem:mean_value_inequality}, we obtain $\|f(\vecnot{x}_2)-f(\vecnot{x}_1)\|\leq M\|\vecnot{x}_2-\vecnot{x}_1\|$, where $M\leq\sup_{s\in[0,1]}\|f'(\vecnot{x}_1+s\vecnot h)\|\leq L$.
This completes the proof.
\end{proof}
%\fi

\begin{lemma}
Let $f \colon X \rightarrow \RealNumbers$ map the Hilbert space $X$ to the real numbers and $A\subseteq X$ be convex.
If $\nabla f(\vecnot{x})$ exists for $\vecnot{x}\in A$, and satisfies $ \| \nabla f(\vecnot{x}) - \nabla f(\vecnot{y}) \| \leq L \| \vecnot{x} - \vecnot{y} \|$ for $\vecnot{x},\vecnot{y}\in A$, then, for all $\vecnot{x},\vecnot{y}\in A$, we have
...
Then, a real functional $f \colon X \rightarrow \RealNumbers$ achieves a \defn{optimization}{local minimum value} $\vecnot{x}_0 \in A$ on $A$ if there is an $\epsilon > 0$ such that, for all $\vecnot{x}\in A$ satisfying $\| \vecnot{x} - \vecnot{x}_0 \| < \epsilon$, we have  $f(\vecnot{x}) \geq f(\vecnot{x}_0)$.
If the bound holds for all $\vecnot{x} \in A$, then the local minimum is also a \defn{functions}{global minimum value} on $A$.
\end{definition}

\begin{theorem}
Let $(X,\|\cdot\|)$ be a normed vector space and $f\colon X \to \mathbb{R}$ be a real functional.
If $\delta f (\vecnot{x}_0;\vecnot{h})$ exists and is negative for some $\vecnot{h} \in X$, then $\vecnot{x}_0$ is not a local minimum value.
\end{theorem}
\begin{proof}
...
Thus, there can be no $\epsilon>0$ satisfying the definition of a local minimum value in  Definition~\ref{definition:minimum_value}.
\end{proof}
...
A functional $f$ is called (strictly) \defn{functions}{concave} if $-f$ is (strictly) convex.
\end{definition}
...
An equivalent condition is that equality in the triangle inequality (i.e., $\| \vecnot{x} + \vecnot{y} \| = \| \vecnot{x} \| + \| \vecnot{y} \|$) for non-zero vectors implies that $ \vecnot{x} = s \vecnot{y} $ for some $s\in \mathbb{R}_{>0}$.
\end{definition}
...
\begin{example}
Let $X$ be an inner-product space. For $\vecnot{x},\vecnot{y} \in X$ and $\lambda \in (0,1)$,
\begin{align*}
...
\end{theorem}
\begin{proof}
Let $\vecnot{x}_0 \in A$ be a point where the functional achieves a local minimum value.
...
Also, if $f$ is strictly convex, then~\eqref{eq:convex_above_tangent} is strict for $\vecnot{x} \neq \vecnot{x}_0$.
Thus,
\[  f(\vecnot{x}) \geq f(\vecnot{x}_0) + \frac{f \left(\vecnot{x}_0 + \lambda (\vecnot{x}-\vecnot{x}_0) \right)- f(\vecnot{x}_0)}{\lambda} \]
...
\begin{corollary}
Let $(X,\|\cdot\|)$ be a normed vector space and $f \colon X \rightarrow \RealNumbers$ be a convex functional on a convex set $A \subseteq X$.
If $f$ is G\^{a}teaux differentiable at $\vecnot{x}_0 \in A$ and $\delta f (\vecnot{x}_0;\vecnot{x}-\vecnot{x}_0) \geq 0$ for all $\vecnot{x} \in A$, then
...
Let $f_i \colon \mathcal{D} \rightarrow \mathbb{R}$ and $h_j \colon \mathcal{D} \rightarrow \mathbb{R}$ be real-valued functions on $\mathcal{D}$ for $i=0,1,\ldots,m$ and $j=1,2,\ldots,p$.
...
Evaluating the objective function at any feasible point automatically gives an upper bound on $p^*$ because
...
\begin{definition}
A point $\vecnot{x}^*$ is called \defn{optimization}{locally optimal} if there is an $\epsilon_0>0$ such that $f_0(\vecnot{x})\geq f_0(\vecnot{x}^*)$ for every $\vecnot{x}\in\mathcal F$ satisfying $\|\vecnot{x}-\vecnot{x}^*\|<\epsilon_0$.
...
A standard constrained optimization problem satisfies the MF constraint qualification at $\vecnot{x}^*$ if the functions $f_i$ and $h_j$ are all continuously differentiable at $\vecnot{x}^*$, $\nabla h_1(\vecnot{x}^*),\ldots,\nabla h_p(\vecnot{x}^*)$ are linearly independent, there exists $\vecnot w\in\mathbb R^n$ such that $\nabla h_j(\vecnot{x}^*)^T\vecnot w=0$ for $j = 1,\ldots,p$, and $\nabla f_i(\vecnot{x}^*)^T\vecnot w<0$ for every $i\in A$.
Redundant affine equalities can be removed before applying this definition.
\end{definition}
...
Before considering its proof, it is useful to discuss the geometric picture underlying its contrapositive statement: if \eqref{eq:KKT1} does not hold for any $\vecnot{\lambda}^* \geq 0$ and $\vecnot{\nu}^* \in \mathbb{R}^p$, then $\vecnot{x}^*$ is not locally optimal.
...
Let $H=\Span\{\nabla h_j(\vecnot{x}^*):j=1,\ldots,p\}$ be the normal space generated by the equality constraints.  Similarly, let $F$ be the cone generated by the normals of the active inequality constraints
\[ F = \left\{ \sum_{i\in A} \lambda_i \nabla f_i (\vecnot{x}^*) \, \middle| \, \lambda_i \geq 0, i \in A   \right\}. \]
Thus, a first-order feasible direction $\vecnot y$ must lie in $H^\perp$ and satisfy $\vecnot u^T\vecnot y\leq0$ for every $\vecnot u\in F$.
...
\begin{proof}
For simplicity, we prove only the case where $h_j (\vecnot{x}) = \vecnot{a}_j^T \vecnot{x} - b_j$ is affine for $j=1,\ldots,p$.
\gpt{Proof-scope omission: Add citation for general case.}
...
&\geq \|\vecnot{y}^*\|^2.
\end{align*}
If~\eqref{eq:KKT1} cannot be satisfied by any $\vecnot\lambda\in S$ and $\vecnot\nu\in\mathbb R^p$, then $\vecnot{y}^* \neq \vecnot{0}$ and $\| \vecnot{y}^* \| > 0$.
...
But, the $\vecnot{y}^*$ direction is only guaranteed to preserve feasibility to first order (i.e., $(P \nabla f_i (\vecnot{x}^*))^T \vecnot{y}^*  \leq 0$) for $i \in A$.
To fix this, first one needs to augment $\vecnot{y}^*$ with a small amount of some vector $\vecnot{w}$ satisfying $(P \nabla f_i (\vecnot{x}^*))^T \vecnot{w} < 0$ for all $i \in A$.
Such a vector exists because of constraint qualification guarantees strict decrease for active inequality constraints and inactive constraints remain feasible by continuity.

%Such a $\vecnot{w}$ lies in the ``interior of the polar cone of $PF$'' and will exist as long as $PF$ does not contain a line.
Then, we can choose some $\delta>0$ such that $(P\nabla f_0 (\vecnot{x}^*))^T (\vecnot{y}^*+\delta \vecnot{w}) \leq -\frac{1}{2} \|\vecnot{y}^* \|^2$. With this modification, the definition of the derivative implies that, for sufficiently small $t$, $\vecnot{x}(t) = \vecnot{x}^* + t (\vecnot{y}^* + \delta \vecnot{w})$ will be a feasible vector satisfying $f_0 (\vecnot{x}(t)) < f_0 (\vecnot{x}^*)$.
\end{proof}
...
\end{align*}
has optimal value $d^*\triangleq\sup_{\vecnot\lambda\ge0,\vecnot\nu}g(\vecnot\lambda,\vecnot\nu)$, and $p^*\geq d^*$.
%has a unique maximum value $d^* \leq p^*$.
This is called \defn{optimization}{weak duality}.
\end{lemma}
...
\end{align*}
For $\vecnot \nu \in \mathbb{R}^p$ and $\vecnot\lambda\ge0$, we have
\begin{align*}
g(\vecnot{\lambda},\vecnot{\nu})
...
&\stackrel{(b)}{\leq}  \inf_{\vecnot{x}\in \mathcal{F}} f_0(\vecnot x ) %+\sum_{i=1}^m \lambda_i f_i(\vecnot x ')
\stackrel{(c)}{\leq} p^*,
\end{align*}
where $(a)$ is implied by $\mathcal{F} \subseteq \mathcal{D}$, $(b)$ follows from $h_j(\vecnot{x}) = 0$ for $\vecnot{x} \in \mathcal{F}$, $f_i(\vecnot{x}) \leq 0$ for $\vecnot{x}\in \mathcal{F}$, and $\lambda_i \geq 0$.
Then, $(c)$ holds by combining the definition of $p^*$ with the fact that $\vecnot{x}\in \mathcal{F}$.
Taking the supremum over dual-feasible multipliers gives $d^*\le p^*$.
If $\mathcal F$ is empty, $p^*=\infty$ and the conclusion is immediate.
\end{proof}
...
Applying Theorem~\ref{theorem:convex_unique_min} to this setup shows that any local minimizer of a convex problem is global.  If an optimum is attained and $f_0$ is strictly convex on the feasible set, then the minimizing point is unique.
...
If a convex optimization problem has a strictly feasible point $\vecnot{x}_0$, meaning $f_i(\vecnot{x}_0)<0$ for all $i$ and $h_j(\vecnot{x}_0)=0$ for all $j$, then strong duality holds.  
If, in addition, $f_0,f_1,\ldots,f_m$ are continuously differentiable and redundant affine equalities have been removed, then MF constraint qualification holds at every feasible point and the KKT conditions are necessary and sufficient for optimality.
\end{theorem}

\begin{example}
For $n$ independent channels with noise power $\alpha_i >0$ and an overall power constraint of $P>0$, the channel input powers $x_i$ that maximize the achievable information rate can be found by solving the convex optimization problem, known as water-filling, given by
\begin{align*}
...
\[ L(\vecnot{x},\vecnot{\lambda},\nu) = - \sum_{i=1}^n \log(x_i + \alpha_i) - \sum_{i=1}^n \lambda_i x_i + \nu \left( -P + \sum_{i=1}^n x_i \right)  \]
and the Lagrangian dual is given by $g(\vecnot{\lambda},\nu)
= \inf_{\vecnot{x}\in \mathcal{D}} L(\vecnot{x},\vecnot{\lambda},\nu)$, where \[\mathcal{D} = \{ \vecnot{x} \in \mathbb{R}^n | x_i > -\alpha_i, i=1,\ldots,n \}\] is the natural domain where the objective is well-defined.

The first-order optimality conditions, for $i=1,2,\ldots,n$, are given by
\[ -\frac{1}{x_i + \alpha_i} - \lambda_i + \nu = 0.
\]
Solving for $x_i$ gives 
$x_i=1/(\nu-\lambda_i)-\alpha_i$ and shows that we must have $\nu>\lambda_i\geq0$.
Also, maximization over $\lambda_i\geq0$ chooses $\lambda_i=0$ when $1/\nu-\alpha_i\geq0$ and otherwise chooses $\lambda_i=\nu-1/\alpha_i$.  Thus $x_i=\max\{0,1/\nu-\alpha_i\}$, with $\nu>0$.
Thus, the expression
...
\label{sec:opt_def_proof}

\subsection*{The Chain Rule for Fr\'echet Derivatives}
\begin{proof}[Proof of Theorem~\ref{thm:frechet_chain_rule}]
...
\end{align*}
We take this opportunity to note that $\|g'(f(\vecnot{x}))\circ f'(\vecnot{x})\| \leq \|g'(f(\vecnot{x}))\| \|f'(\vecnot{x})\| < \infty$ because $\|f'(\vecnot{x})\| < \infty$ and $\|g'(f(\vecnot{x}))\| <\infty$. 

Since $\lim_{\vecnot{h} \to \vecnot{0}} \| \phi (\vecnot{h}) \|_Y / \|\vecnot{h}\|_X = 0$, there is a $t>0$ such that $\|\phi(\vecnot{h})\|_Y \leq (\|f'(\vecnot{x})\|+1) \|\vecnot{h}\|_X$ if $\|\vecnot{h}\|_X < t$.
Now, set $u(\vecnot h)=f'(\vecnot x)(\vecnot h)+\phi(\vecnot h)$ and observe that, for sufficiently small $\vecnot h$, we have $\| u(\vecnot h)\|_Y\leq 2(\|f'(\vecnot x)\|+1)\|\vecnot h\|_X$.
Using this, we can write
\begin{align*}
\frac{\|\rho(\vecnot{h})\|_Z}{\|\vecnot{h}\|_X}
&= \frac{\| \psi \big(f'(\vecnot{x})(\vecnot{h}) + \phi(\vecnot{h}) \big) + g'(\vecnot{y})(\phi(\vecnot{h})) \|_Z}{\|\vecnot{h}\|_X} \\
&\leq 2 (\|f'(\vecnot{x})\|+1)  \frac{\| \psi \big(u(\vecnot{h}) \big) \|_Z}{ \| u(\vecnot{h}) \|_Y } + \frac{\|  g'(\vecnot{y})(\phi(\vecnot{h})) \|_Z}{\|\vecnot{h}\|_X} \\
& \leq 2 (\|f'(\vecnot{x})\|+1) \frac{\| \psi \big( u(\vecnot{h}) \big) \|_Z}{\| u(\vecnot{h}) \|_Y} + \frac{\|  g'(\vecnot{y})\| \|\phi(\vecnot{h}) \|_Y}{\|\vecnot{h}\|_X} \\
& \leq 2 (\|f'(\vecnot{x})\|+1) \frac{\| \psi \big( u(\vecnot{h}) \big) \|_Z}{\| u(\vecnot{h}) \|_Y} + \frac{\|  g'(\vecnot{y})\| \|\phi(\vecnot{h}) \|_Y}{\|\vecnot{h}\|_X},
\end{align*}
where the first quotient is taken to be zero when $u(\vecnot h)=\vecnot0$.
If it is not zero, $u(\vecnot{h})=f'(\vecnot{x}) (\vecnot{h}) + \phi(\vecnot{h}) \to \vecnot{0}$ as $\vecnot{h}\to\vecnot{0}$ and it follows that the limit of the RHS exists and equals 0 as $\vecnot{h}\to\vecnot{0}$.
...
\subsection*{Banach Mean Value Inequality}

\begin{proof}[Proof of Theorem~\ref{theorem:mean_value_inequality}]
By assumption, \(\vecnot{h}\neq 0\) and we define $F(s)=f(\vecnot{x}_1+s\vecnot{h})$ for $s\in[0,1]$,
and let $L
=
\sup_{s\in[0,1]}
\left\|
\delta f(\vecnot{x}_1+s\vecnot{h};\vecnot{h})
\right\|$.
Thus, we let $L=M\|\vecnot{h}\|$ and assume $L<\infty$ because otherwise, the desired inequality is immediate.

For the sake of contradiction, suppose that $\|f(\vecnot{x}_2)-f(\vecnot{x}_1)\| > M\|\vecnot{x}_2-\vecnot{x}_1\|$ which is equivalent to $\|F(1)-F(0)\|>L$.
Then, we can choose \(K\) such that $L<K<\|F(1)-F(0)\|$.
Now, we will construct a nested sequence of closed intervals $I_n=[a_n,b_n]\subseteq[0,1]$ such that $b_n-a_n=2^{-n}$ and $\|F(b_n)-F(a_n)\|>K(b_n-a_n)$.
For \(n=0\), take \(I_0=[0,1]\).

Suppose that \(I_n=[a_n,b_n]\) has been constructed, and let $m_n=\frac{a_n+b_n}{2}$.
By the triangle inequality, $\|F(b_n)-F(a_n)\|
\leq
\|F(b_n)-F(m_n)\|
+
\|F(m_n)-F(a_n)\|$.
Since $\|F(b_n)-F(a_n)\|>K(b_n-a_n)$,
either $\|F(m_n)-F(a_n)\|>K(m_n-a_n)$
or $\|F(b_n)-F(m_n)\|>K(b_n-m_n)$ must hold.
Choose \(I_{n+1}\) to be a half of \(I_n\) for which the
corresponding inequality holds.

Thus, we obtain nested intervals \(I_n=[a_n,b_n]\) satisfying $b_n-a_n=2^{-n}$
and $\|F(b_n)-F(a_n)\|>K(b_n-a_n)$.
Since the intervals are nested and their lengths tend to zero, there is
a unique point $s_*\in\bigcap_{n=0}^{\infty} I_n$.
Let $\vecnot{x}_*=\vecnot{x}_1+s_*\vecnot{h}.$

Since $s_*\in[a_n,b_n]$, the triangle inequality gives
\[
\|F(b_n)-F(a_n)\|
\leq
\|F(b_n)-F(s_*)\|+\|F(s_*)-F(a_n)\|.
\]
Thus, for each $n$, there must be an endpoint $s_n\in\{a_n,b_n\}$ with $s_n\neq s_*$ such that
\[
\frac{\|F(s_n)-F(s_*)\|}{|s_n-s_*|}>K.
\]
Indeed, otherwise the triangle inequality would imply $\|F(b_n)-F(a_n)\|\leq K(b_n-a_n)$ which is a contradiction.

Since $s_n,s_*\in I_n$ and $b_n-a_n=2^{-n}$, we have
$|s_n-s_*|\leq 2^{-n}$ and hence $s_n\to s_*$.
Let $t_n=s_n-s_*$.
Then, $t_n\to 0$ and
\[
K
<
\frac{\|F(s_*+t_n)-F(s_*)\|}{|t_n|}
=
\left\|
\frac{f(\vecnot{x}_*+t_n\vecnot{h})-f(\vecnot{x}_*)}{t_n}
\right\| \to
\| \delta f(\vecnot{x}_*;\vecnot{h}) \|
\]
because the G\^{a}teaux differential $\delta f(\vecnot{x}_*;\vecnot{h})$ exists. 
Therefore, $K \leq \|\delta f(\vecnot{x}_*;\vecnot{h})\|
\leq L$, which contradicts $L<K$ and thus
\[
\|f(\vecnot{x}_2)-f(\vecnot{x}_1)\|
\leq
M\|\vecnot{x}_2-\vecnot{x}_1\|. \qedhere
\]
\end{proof}
