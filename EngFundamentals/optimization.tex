...
exists.
If $J(\vecnot{x})$ exists, then its operator norm $\| J(\vecnot{x}) \| < \infty$ because it is finite-dimensional.
A sufficient condition for differentiability is that each partial derivative exists and is continuous in $\vecnot{x}$ at $\vecnot{x}=\vecnot{x}_0$.
...
&\stackrel{(b)}{\leq}  f_0(\vecnot x ')+\sum_{i=1}^m \lambda_i f_i(\vecnot x ')
\stackrel{(c)}{\leq} p^*,
\end{align*}
...
Now, set $\vecnot u(\vecnot h)=f'(\vecnot x)(\vecnot h)+\phi(\vecnot h)$ and observe that, for sufficiently small $\vecnot h$, we have $\|\vecnot u(\vecnot h)\|_Y\leq 2(\|f'(\vecnot x)\|+1)\|\vecnot h\|_X$.
Using this, we can write
\begin{align*}
...
& \leq 2 (\|f'(\vecnot{x})\|+1) \frac{\| \psi \big( u(\vecnot{h}) \big) \|_Z}{\| u(\vecnot{h}) \|_Y} + \frac{\|  g'(\vecnot{y})\| \|\phi(\vecnot{h}) \|_Y}{\|\vecnot{h}\|_X},
\end{align*}
where the first quotient is taken to be zero when $\vecnot u(\vecnot h)=\vecnot0$.
