...
\begin{definition}
  ...
  For $y\in Y$, we say that $\lim_{x\to z} f(x) = y$ (i.e., the limit as $x \to z$ of $f(x)$ equals $y$) if $f(x_n) \to y$ for all sequences $x_n \in X$ such that $x_n \to y$.
  If there is no such $y\in Y$, then we say the limit does not exist.
  
  The proof of Theorem~\ref{thm:cont_fun_limit} shows that this is equivalent to: for any $\epsilon> 0$, there is a $\delta >0$ such that, for all $x\in X$ satisfying $\|x - z\|< \delta$, we have $\| y - f(x) \| < \epsilon$.
\end{definition}
