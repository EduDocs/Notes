...
A statement, or \textbf{proposition}, is te content of an assertion.
...
The conjunction of $P$ and $Q$, denoted by $P \wedge Q$, is precisely defined by
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
$P$ & $Q$ & $P \wedge Q$ \\
...
There are a number of standard sets worth mentioning: the integers $\mathbb{Z}$, the real numbers $\mathbb{R}$, and the complex numbers $\mathbb{C}$.
...
\begin{definition}
We say that $A$ is a \defn{set theory}{subset} of $B$ (denoted $A\subseteq B$) if $x\in A$ implies $x\in B$.
This means that
...
\end{equation*}
\end{example}
The countably infinite product of $X$ is associated with infinite sequences on $X$ and is denoted either by $X^\infty$ or $X^\omega$.
...
For example, let $A$ be a set of people and $P(x,y)$ be the statement ``$x$ has the same birthday as $y$''.
...
In this case, the equivalence classes can be associated with the days of the year.
...
Nowadays, every teacher warns his students that one must be careful because the world is filled with not so ``nice'' functions.
...
The \emph{range} of $f$ is the subset of $Y$ which is actually achieved by $f$ or  $\{f(x) \in Y | x\in X \}$.
