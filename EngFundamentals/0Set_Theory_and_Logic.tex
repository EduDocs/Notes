...
The \defn{logic}{conditional connective} from $P$ to $Q$, denoted $P \rightarrow Q$, is a logical statement that is read ``if $P$ then $Q$".
...
In this context, the term \defn{logic}{consistent} means that all logical implications generated by the axioms will be correct, the term \defn{logic}{complete} means that any true logical implication can be generated by applying the axioms, and the term \defn{logic}{decidable} means that there is a terminating method that always determines whether a postulated implication is valid or invalid.
...
Likewise, in predicate logic, the statement $P(x)=``x$ is a person'' is called a predicate and $x$ is called a \defn{logic}{free variable}.
...
If $U=\emptyset$ is the empty set, then $\forall x \in U, P(x)$ is vacuously true because there are no elements in $U$ to test with $P(x)$.
...
These definitions also imply two new inference rules,
\begin{align*}
\forall x\in U, P(x) &\Rightarrow \exists x\in U, P(x) \\
\exists x\in U, P(x) &\Rightarrow P(x_0),
\end{align*}
where $x_0 \in U$ is chosen so that $P(x_0)$ is true and $\Rightarrow$ indicates that these statements hold for all sets $U$ and predicates $P$.
...
\end{gather*}
Moreover, these implications can be proven using the new inference rules.
...
\[ \begin{array}{ccccccc}
  ...
  \end{array}
\]
All of these implications follow from the single variable inference rule $\forall x, P(x) \Rightarrow \exists x, P(x)$ except for two: $\exists x, \forall y, P(x,y) \Rightarrow \forall y, \exists x, P(x,y)$ and its symmetric pair.

To understand this rule, consider an example where $x$ is in a set $I$ of images and $y$ is in a set $C$ of colors.
...
If $|A| = \infty$ and the set is not countably infinite, then $A$ is called \defn{set theory}{uncountable infinite}.
\end{definition}
...
On the other hand, in the Zermelo-Fraenkel axiomatic formulation of set theory, it is a theorem that no set contains itself.
