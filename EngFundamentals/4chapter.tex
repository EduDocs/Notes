...
The \defn{linear transform}{transpose} of $T$ is the mapping $U : W^* \rightarrow V^*$ defined by $f(\vecnot{v})=g(T\vecnot{v})\in V^*$ for all $g\in W^*$.
...
= \sup_{\vecnot{u} \in V, \left\| \vecnot{v} \right\| = 1 }
...
\[ \| f \| \triangleq \sup_{\vecnot{u}\in V - \{ \vecnot{0} \}} \frac{f(\vecnot{u})}{\| \vecnot{u} \|} = \| v \|. \]
Since $f$ is continuous, it follows that $\| f \| < \infty$ and $\| v \| < \infty$.
The Cauchy-Schwarz inequality shows that $\langle \vecnot{w} | \vecnot{v} \rangle \leq \| w \| \| v \|$ so that $f$ is continuous (and therefore linear) because $\| v \| < \infty$.
Then, 
\begin{equation*}
...
Then, the \defn{inner-product space}{adjoint} is the unique linear transformation $T^*$ on $V$ such that
\begin{equation*}
...
Using a similar argument, one can show that $\left[ R_{T^*} \right]^* = N_T$.
\end{proof}
