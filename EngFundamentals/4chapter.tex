...
The induced operator norm also has a new property that follows naturally from the definition.
Notice that 
...
A norm on linear transforms is called \textbf{submultiplicative} if $\| T \vecnot{u} \| \leq \| T \| \| \vecnot{u} \|$ for all $\vecnot{u}\in V$.
Checking the special case of $\vecnot{u}=\vecnot{0}$, we find that all induced operator norms are submultiplicative.
From this, one also gets a submultiplicative norm for the algebra of linear operators because
\begin{equation*}
...
There are four fundamental subspaces of a linear transformation $T \colon V \rightarrow W$.
...
\begin{theorem}
Let $T \colon V \rightarrow W$ be a bounded linear transformation between two Hilbert spaces $V$ and $W$, and let $R_T$ and $R_{T^*}$ be closed.
Then,
\begin{enumerate}
