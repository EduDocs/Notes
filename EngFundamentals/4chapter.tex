...
For the standard Schauder basis $\{\vecnot{e}_1,\vecnot{e}_2,\ldots\}$, let $T \colon V \rightarrow V$ be the linear transformation that satisfies $T \vecnot{e}_i = i^{-1} \vecnot{e}_i$ for $i=1,2,\ldots$.
...
If $g \in W^*$ is a linear functional on $W$ (i.e., $g \colon W \rightarrow F$), then $f\colon V \to F$ (i.e., $f\in V^*$) defined by $\vecnot{v} \mapsto g(T\vecnot{v})$ is a linear functional on $V$.
The \defn{linear transform}{transpose} of $T$ is the implied mapping $U \colon W^* \rightarrow V^*$ that satisfies $f(\vecnot{v})=g(T\vecnot{v})\in V^*$ for all $g\in W^*$ and all $\vecnot{v}\in V$.
...
If $V$ has a norm, the we instead use $V^*$ to denote the continuous dual (i.e., the set of continuous linear functionals).
...
\begin{theorem}
For a Banach space $V$ with a submultiplicative operator norm $\| \cdot \|$, let $T \colon V \rightarrow V$ a linear operator with $\left\| T \right\| < 1$.
...
The mapping $R$ is a conjugate-linear isometric (i.e., $\| f \| = \| R(f) \|$) bijection and is linear over $\mathbb R$.
It provides a canonical self-duality we typically pretend $V=V^*$, but retaining the distinction between $V$ and $V^*$ avoids conjugation errors.
