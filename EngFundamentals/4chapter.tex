...
The function $T$ from $V$ into $W$ is called \defn{linear transform}{invertible} if there exists a function $U$ from $W$ to $V$ such that $UT$ is the identity function on $V$ and $TU$ is the identity function on $W$.
...
The induced \defn{linear transform}{operator norm} of $T$ is given by
\begin{equation*}
\left\| T \right\|
...
Let $T$ be a linear operator on $V$ and let $A$ be the matrix of $T$ in the ordered basis $\mathcal{B}$.
...
The matrix $\left( A^H A \right)^{-1} A^H$ is an example of a \defn{matrix}{pseudoinverse}, sometimes called the Moore-Penrose pseudoinverse.
...
The \defn{linear transformation}{pseudoinverse} $T^{\dagger}$ is the transformation mapping $\vecnot{w}$ to the minimizing $\vecnot{v}$ for each $\vecnot{w} \in W$.
\end{definition}
