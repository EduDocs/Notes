...
\gpt{Omission: the theorem claims that $T^*$ is bounded, but the proof establishes only existence, linearity, and uniqueness.  Add the estimate $\|T^*\vecnot w\|=\|f_{\vecnot w}\|\leq\|T\|\,\|\vecnot w\|$, which gives $\|T^*\|\leq\|T\|$ (and applying the same argument to $T^*$ yields equality).}
