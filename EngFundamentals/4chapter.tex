...
% \gpt{Chapter-level feedback: Duality, operator norms, adjoints, and pseudoinverses are excellent choices for both signal processing and quantum computing.  The chapter would be easier to navigate if it more clearly separated finite-dimensional matrix facts from infinite-dimensional operator theory.  In particular, the matrix 2-norm derivation depends on eigenvalues and the Hermitian spectral theorem, which appear only in later chapters; consider moving ``Matrix Norms'' after the spectral theorem or revisiting it after the SVD.  Conversely, place the general Riesz theorem before the Optimization chapter if that chapter is to use Hilbert-space gradients.}
...
If $V$ is a Banach space and $L(V,V)$ is equipped with the induced operator norm, then the space of bounded linear operators on $V$ forms a Banach algebra.
\end{example}
...
\begin{example}
Consider the normed vector space $V$ of semi-infinite real sequences $\mathbb{R}^\omega$ with finitely many non-zero terms under the $\ell^2$ norm.
For the standard basis $\{\vecnot{e}_1,\vecnot{e}_2,\ldots\}$, let $T \colon V \rightarrow V$ be the linear transformation that satisfies $T \vecnot{e}_i = i^{-1} \vecnot{e}_i$ for $i=1,2,\ldots$.
Likewise, let the linear transformation $U \colon V \rightarrow V$ satisfy $U \vecnot{e}_i = i \vecnot{e}_i$ for $i=1,2,\ldots$.
...
\dim V^* = \dim V.
\end{equation*}
Thus, $V$ and $V^*$ are isomorphic.
However, the identification between them is not canonical in general.
To identify $V$ with $V^*$, one must choose a basis for $V$ and then derive the dual basis for $V^*$.
...
Then, for all $\vecnot{v} \in V$, there exists $s_1,\ldots,s_n$ such that $\vecnot{v} = \sum_{i=1}^n s_i \vecnot{v}_i$.
For any linear functional $f\colon V \to F$, we have
\[ f(\vecnot{v}) = f\left( \sum_{i=1}^n s_i \vecnot{v}_i \right) = \sum_{i=1}^n s_i f(\vecnot{v}_i) \] 
and $f$ is defined by the values it takes on the basis.
Thus, the space of linear functionals has dimension $\dim V^* = n$.

By Theorem~\ref{theorem:UniqueLinearTransformation}, there is a unique linear functional $f_i$ on $V$ such that
\begin{equation*}
...
These functionals are linearly independent because
\begin{equation*}
f = \sum_{i=1}^n t_i f_i ,
\end{equation*}
satisfies
\begin{equation*}
f \left( \vecnot{v}_j \right) = \sum_{i=1}^n t_i f_i \left( \vecnot{v}_j \right)
= \sum_{i=1}^n t_i \delta_{ij} = t_j .
\end{equation*}
In particular, if $f$ is the zero functional, $f \left( \vecnot{v}_j \right) = 0$ for $j = 1, \ldots, n$ and hence the scalars $\{ t_j \}$ must all equal $0$.
It follows that the functionals $f_1, \ldots, f_n$ are linearly independent.
We conclude that $\mathcal{B}^* = f_1, \ldots, f_n$ forms a basis for $V^*$, the \defn{vector space}{dual basis} of $\mathcal{B}$.
...
f = \sum_{i=1}^n t_i f_i .
\end{equation*}
Furthermore, by construction, we must have $t_j = f \left( \vecnot{v}_j \right)$ for $j = 1, \ldots, n$.
Similarly, if
\begin{equation*}
...
\begin{equation*}
f_j \left( \vecnot{v} \right) = \sum_{i=1}^n s_i f_j \left( \vecnot{v}_i \right)
...
\begin{equation*}
\vecnot{v} = \sum_{i=1}^n f_i \left( \vecnot{v} \right) \vecnot{v}_i . \qedhere
\end{equation*}
\end{proof}
...
Let $V,W$ be vector spaces over $F$ and let $T \colon V \rightarrow W$ be a linear transform.
Assume $g \in W^*$ is a linear functional on $W$ and $f \in V^*$ is a linear functional on $V$.
The \defn{linear transform}{transpose} of $T$ is the implied mapping $U \colon W^* \rightarrow V^*$ that maps $g$ to the unique $f$ satisfying $f(\vecnot{v})=g(T\vecnot{v})$ for all $\vecnot{v}\in V$.
...
For vector spaces without topological structure, we use $V^*$ to denote the algebraic dual (i.e., the set of linear functionals).
If $V$ has a norm, then we instead use $V^*$ to denote the continuous dual (i.e., the set of continuous linear functionals).
For infinite dimensional Hilbert spaces, we will see later that the notion of the dual space and adjoint (i.e., transpose) simplifies because one can identify $V=V^*$ via isomorphism.
...
The induced \defn{linear transform}{operator norm} of $T$ is defined to be
\begin{equation*}
\left\| T \right\|
...
Now, we have a norm for the subspace of bounded linear transforms $\{T\in L(V,W):\|T\|<\infty\}$.
...
\end{theorem}
\begin{proof}
Suppose that $T$ is bounded so that $\left\| T \vecnot{v} \right\| \leq M \left\| \vecnot{v} \right\|$ for some $M<\infty$ and all $\vecnot{v} \in V$.
Let $\vecnot{v}_1, \vecnot{v}_2, \ldots$ in $V$ be any convergent sequence and let $\vecnot{v}$ be its limit.
Then,
\[ \| T\vecnot v_i-T\vecnot v\|\le M\|\vecnot v_i-\vecnot v\|\to0 . \]
% \begin{equation*}
...
\end{equation*}
Let $C = \max_{1 \leq i \leq n} \left\| T \vecnot{v}_i \right\|_W$.
Then,
\begin{equation*}
\begin{split}
\left\| T \vecnot{v} \right\|_W &= \left\| T \left( s_1 \vecnot{v}_1 + \cdots + s_n \vecnot{v}_n \right) \right\|_W \\
& \leq |s_1| \left\| T \vecnot{v}_1 \right\|_W + \cdots + |s_n| \left\| T \vecnot{v}_n \right\|_W \\
&\leq C \left( |s_1| + \cdots + |s_n| \right) .
\end{split}
\end{equation*}
By Lemma~\ref{lemma:FiniteDimNormsEquiv}, there is a real $m$ such that  $|s_1| + \cdots + |s_n| \leq \frac{1}{m} \left\| \vecnot{v} \right\|_V$ and
\begin{equation*}
\left\| T \vecnot{v} \right\|_W \leq \frac{C}{m} \left\| \vecnot{v} \right\|_V . \qedhere
\end{equation*}
\end{proof}
...
\begin{theorem}
For a Banach space $V$ that has a submultiplicative operator norm $\| \cdot \|$, let $T \colon V \rightarrow V$ be a linear operator with $\left\| T \right\| < 1$.
...
and this goes to zero as $m\rightarrow \infty$.

For any fixed $\vecnot{v}\in V$, we note that $A_n \vecnot{v}$ converges in $V$ because \[\| A_n \vecnot{v} - A_m \vecnot{v} \| \leq \| A_n - A_m \| \| \vecnot{v}\|. \]
This identifies a pointwise limit for the $A_n$ mapping.
Since $\| A_n \|$ stays bounded, the limit of $A_n$ inherits linearity and is bounded.
Thus, $\lim_{n\rightarrow \infty} A_n$ exists and equals a bounded linear operator.
...
\begin{equation*}
\max_{\| \vecnot{v} \|=1} \left\| A \vecnot{v} \right\|_{2}^{2}
= \max_{\| \vecnot{v} \|=1} \vecnot{v}^H A^H A \vecnot{v} .
\end{equation*}
One can solve this via constrained optimization by introducing the Lagrangian
\begin{equation*}
J = \vecnot{v}^H A^H A \vecnot{v} - \lambda \vecnot{v}^H \vecnot{v} .
\end{equation*}
The stationary condition with respect to $\vecnot{v}$ shows that
\begin{equation*}
A^H A \vecnot{v} = \lambda \vecnot{v} .
\end{equation*}
This implies that $\vecnot{v}$ must be an eigenvector of the matrix $A^H A$.
...
The \defn{matrix}{Frobenius norm} of $A\in F^{m\times n}$ is given by
\begin{equation*}
...
%The Cauchy--Schwarz inequality shows that $\tinner{ \vecnot{w} }{ \vecnot{v} } \leq \| \vecnot{w} \| \| \vecnot{v} \|$ so that $f$ is continuous (and therefore linear) because $\| \vecnot{v} \| < \infty$.
Then, 
\begin{equation*}
...
\end{equation*}
Since this is true for each $\vecnot{v}_j$, they agree on $\Span\{\vecnot v_j\}$ by linearity and they agree on its closure $V$ by continuity.
Thus, $f = f_{\vecnot{v}}$.
Now, consider any $\vecnot{v}' \in V$ such that $\inner{ \vecnot{w} }{ \vecnot{v} } = \inner{ \vecnot{w} }{ \vecnot{v}' }$ for all $\vecnot{w} \in V$.
...
The isomorphism can be shown by verifying that $R( s f_1 + f_2 ) = \overline{s} R(f_1) + R(f_2)$.

For complex spaces, the mapping $R$ is a conjugate-linear isometric (i.e., $\| f \| = \| R(f) \|$) bijection between $V^*$ and $V$.
For real spaces, it is a linear isometric isomorphism over $\mathbb R$)
It provides a canonical self-duality and we typically pretend $V=V^*$, but retaining the distinction between $V$ and $V^*$ avoids conjugation errors.

\begin{theorem}
Let $V$ and $W$ be Hilbert spaces, and assume $T \colon V \rightarrow W$ is a continuous linear transformation.
Then, the \defn{inner-product space}{adjoint} is the unique bounded linear transformation $T^* \colon W \to V$ such that
\begin{equation*}
...
\end{split}
\end{equation*}
Since this holds for all $\vecnot{v} \in V$, we gather from Lemma~\ref{lemma:DualSpaceSeparatesPoints} that $T^* \left( s \vecnot{w}_1 + \vecnot{w}_2 \right) = s T^* \vecnot{w}_1 + T^* \vecnot{w}_2$.
Therefore, $T^*$ is linear.

To verify that $T^*$ is bounded, we start by writing
\[  |\inner{ \vecnot{v} }{ T^* \vecnot{w} }| = |\inner{ T \vecnot{v} }{ \vecnot{w} }| \leq \|T \vecnot{v}\| \|\vecnot{w}\| \leq \| T \| \|\vecnot{v}\| \|\vecnot{w}\|.  \]
Then, we substitute $\vecnot{v}=T^* \vecnot{w}$ into the far LHS and the far RHS to see that
\[  |\inner{ \vecnot{v} }{ T^* \vecnot{w} }| = |\inner{ T^* \vecnot{w} }{ T^* \vecnot{w} }| = \| T^* \vecnot{w}\|^2 \leq \| T \|  \|T^* \vecnot{w}\| \|\vecnot{w}\|. \]
Dividing by $\|T^* \vecnot{w}\| \|\vecnot{w}\|$ and optimizing over $\vecnot{w} \neq \vecnot{0}$ shows that $\|T^* \| \leq \|T \|$.
...
The same is true in infinite-dimensional Hilbert spaces for every bounded linear operator.  Unbounded operators require additional domain hypotheses and need not have an everywhere-defined adjoint.
...
\neq \vecnot{0}.
\end{equation*}
Since $\vecnot w_o\perp R_T$, for every $\vecnot v\in V$ we have $\inner{\vecnot v}{T^*\vecnot w_o}=\inner{T\vecnot v}{\vecnot w_o}=0$. 
Hence $T^*\vecnot w_o=\vecnot0$ and $\vecnot w_o\in N_{T^*}$.
But, we also have
\begin{equation*}
...
%\item There exists $\vecnot{u} \in W$ such that $T^* \vecnot{u} = \vecnot{0}$ and $\tinner{ \vecnot{w} }{ \vecnot{u} } \neq 0$.
%\end{itemize}
%\end{remark}
...
For Hilbert spaces $V$ and $W$, let $T\colon V\to W$ be bounded linear transformation.
Then, for $\vecnot w\in W$, a common goal is to find $\vecnot v\in V$ that minimizes $\left\| T \vecnot{v} - \vecnot{w} \right\|^2$.

\begin{theorem} \label{theorem:hilbert_lsq}
A vector $\vecnot{v} \in V$ minimizes $\left\| T \vecnot{v} - \vecnot{w} \right\|$ if and only if
\begin{equation*}
T^* T \vecnot{v} = T^* \vecnot{w}.
\end{equation*}
\end{theorem}
\begin{proof}
For any $\vecnot{h}\in V$, we have
\begin{align*}
\left\|T(\vecnot{v}+\vecnot{h})-\vecnot{w}\right\|^2
&=
\left\|T\vecnot{v}-\vecnot{w}\right\|^2
+2\,\Real\left\langle T\vecnot{h},T\vecnot{v}-\vecnot{w}\right\rangle
+\left\|T\vecnot{h}\right\|^2 \\
&=
\left\|T\vecnot{v}-\vecnot{w}\right\|^2
+2\,\Real\left\langle \vecnot{h},T^*(T\vecnot{v}-\vecnot{w})\right\rangle
+\left\|T\vecnot{h}\right\|^2.
\end{align*}
Thus, if $T^*T\vecnot{v}=T^*\vecnot{w}$, then
\[
\left\|T(\vecnot{v}+\vecnot{h})-\vecnot{w}\right\|^2
=
\left\|T\vecnot{v}-\vecnot{w}\right\|^2+\left\|T\vecnot{h}\right\|^2
\geq
\left\|T\vecnot{v}-\vecnot{w}\right\|^2,
\]
so $\vecnot{v}$ is a minimizer.

Conversely, suppose that $\vecnot{v}$ is a minimizer.  For any $\vecnot{h}\in V$ and $t\in\mathbb{R}$,
\[
\left\|T(\vecnot{v}+t\vecnot{h})-\vecnot{w}\right\|^2
=
\left\|T\vecnot{v}-\vecnot{w}\right\|^2
+2t\,\Real\left\langle \vecnot{h},T^*(T\vecnot{v}-\vecnot{w})\right\rangle
+t^2\left\|T\vecnot{h}\right\|^2.
\]
Since this is minimized at $t=0$, we must have
\[
\Real\left\langle \vecnot{h},T^*(T\vecnot{v}-\vecnot{w})\right\rangle=0
\]
for every $\vecnot{h}\in V$.  Taking
$\vecnot{h}=T^*(T\vecnot{v}-\vecnot{w})$ gives
\[
T^*(T\vecnot{v}-\vecnot{w})=\vecnot{0},
\]
or equivalently $T^*T\vecnot{v}=T^*\vecnot{w}$.
\end{proof}

\begin{corollary} \label{corollary:hilbert_lsq}
If $\vecnot{w}\in R_T\oplus R_T^\perp$, then there exists
$\vecnot{v}\in V$ minimizing $\|T\vecnot{v}-\vecnot{w}\|$.
\end{corollary}
\begin{proof}
Write $\vecnot{w}=\hat{\vecnot{w}}+\vecnot{w}_\perp$, where
$\hat{\vecnot{w}}\in R_T$ and $\vecnot{w}_\perp\in R_T^\perp$.
Choose $\vecnot{v}\in V$ such that $T\vecnot{v}=\hat{\vecnot{w}}$.
Then
\[
T\vecnot{v}-\vecnot{w}=-\vecnot{w}_\perp\in R_T^\perp=N_{T^*},
\]
so $T^*T\vecnot{v}=T^*\vecnot{w}$, and the result follows from Theorem~\ref{theorem:hilbert_lsq}.
\end{proof}
