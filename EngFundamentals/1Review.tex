...
\begin{definition}
A \defn{metric spaces}{metric} on a set $X$ is a function
\begin{equation*}
...
\begin{example}
Given $\vecnot{x} = (x_1, \ldots, x_n), \vecnot{y} = (y_1, \ldots, y_n) \in \RealNumbers^n$, the \defn{metric spaces}{euclidean metric} $d$ on $\RealNumbers^n$ is defined by the equation
\begin{equation*}
...
Then, elements of $X$ are called \defn{metric spaces}{points} and the number $d(x,y)$ is called the \defn{metric spaces}{distance} between $x$ and $y$.
Let $\epsilon > 0$ and consider the set $B_d(x,\epsilon) = \left\{ y \in X | d(x,y) < \epsilon \right\}$.
This set is called the \defn{metric spaces}{$d$-open ball} of radius $\epsilon$ centered at $x$.
...
\begin{definition}
A \defn{metric spaces}{sequence} of elements from a set $X$ is an infinite list $x_1,x_2,\ldots$ where $x_i \in X$ for all $i\in \mathbb{N}$.
...
This sequence \defn{metric spaces}{$d$-converges} to $x\in X$ if, for any $\epsilon >0$, there is natural number $N$ such that $d(x,x_n) < \epsilon$ for all $n>N$.
\end{definition}

\begin{definition}
A sequence $x_1,x_2,\ldots$ in $(X,d)$ is a \defn{metric spaces}{Cauchy sequence} if, for any $\epsilon >0$, there is a natural number $N$ (depending on $\epsilon$) such that, for all $m,n > N$,
\begin{equation*}
...
The set $W$ is called \defn{metric spaces}{$d$-open} if, for every $w\in W$, there is an $\epsilon>0$ such that $B_d (w,\epsilon) \subseteq W$.
\end{definition}
...
Then, $f$ is \defn{metric spaces}{$d$-continuous} at $x_0$ if, for any $\epsilon> 0$, there is a $\delta >0$ such that 
...
A subset $A \subset X$ is called an \defn{topology}{open set} of $X$ if $A \in \mathcal{J}$.
...
A function $f: X \rightarrow Y$ is \defn{topology}{continuous} if for each open subset $O \subset Y$, the set $f^{-1} (O)$ is an open subset of $X$.
\end{definition}
