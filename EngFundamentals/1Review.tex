...
We begin this chapter with a discussion of key mathematical concepts and motivate how they can be generalized to address important challenges.
...
This sequence converges to $x$ if, for any $\epsilon > 0$, there exists a natural number $N$ such that $| x_n - x | < \epsilon$ for all $n > N$.
...
\end{itemize}
One scenario where convergence plays an important role is in the design of iterative methods applied to optimization objectives.
...
These definitions are clearly empowering when dealing with the real numbers.
Yet, we are interested in exploring spaces that contain objects such as time series, images, feature vectors, polynomial, matrices, and functions.
This raises an important question: How can we extend convergence and continuity to more intricate spaces?
This step is key in better understanding the structure of the abstract spaces and, also, in designing algorithms and methods attuned to these spaces.

A first step in developing a more general theory is to identify what makes these definitions work for the real numbers.
Based on the insights we gain, we can then extract key attributes and see how they can help define similar notions for abstract spaces.
We begin with some pertinent questions.
First off, what are the real numbers and why do we use them all the times, as opposed to, say, the rational numbers?
...
Is there a notion of convergent sequence for which we do not need to pocess and explicit characterization of its limit beforehand?
Finally, both definitions above rely on proximity, respectively $|x_n - x|$ and $|f(x) - f(x_0)|$.
We have a pretty good grasp of the distance between two points on the real line.
How can we define distances between points in more general spaces?
These are the questions we seek to address in this chapter, in the hope of leveraging our familiarity with real numbers into understanding more general spaces.
