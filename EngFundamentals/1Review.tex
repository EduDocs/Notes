...
\begin{definition}
An \defn{topology}{isometry} is a mapping $\phi : X \rightarrow Y$ between two metric spaces $(X,d_X)$ and $(Y,d_Y)$ that is distance preserving (i.e., it satisfies $d_X (x,x') = d_Y \left(\phi(x),\phi(x')\right)$ for all $x,x'\in X$).
\end{definition}
...
%Then, we notice that the set $A = \{x_1,x_2,\ldots \}$ is closed and therefore compact by \ref{thm:CompactClosedSubsetCompact}.
...
\begin{definition}
The \defn{set theory}{supremum} (or least upper bound) of $X\subseteq \RealNumbers$, denoted $\sup X$, is the smallest extended real number $M \in \RealNumbers \cup \{-\infty,\infty\}$ such that $x\leq M$ for all $x\in X$.
...
\begin{definition}
The \defn{set theory}{infimum} (or greatest lower bound) of $X\subseteq \RealNumbers$, denoted $\inf X$, is the largest extended real number $m \in \RealNumbers \cup \{-\infty,\infty\}$ such that $x\geq m$ for all $x\in X$.
It is always well-defined and equals $\infty$ if $X=\emptyset$.
\end{definition}
