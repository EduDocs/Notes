...
The \boldmath%
\defn{metric space}{limit}~\textbf{of $f$ at $x_0$}\unboldmath~equals $y$ in this case and we say the limit exists if this holds for some $y\in Y$.
...
Let $h:\overline{\mathbb R}\to[-1,1]$ be defined by $h(x)=x/(1+|x|)$ for $x\in\mathbb R$ and $h(\pm\infty)=\pm1$.
Then, the extended real line equipped with the metric $d_{\overline{\mathbb R}}(x,y)=|h(x)-h(y)|$ defines the standard metric space of extended real numbers.
Later, we will see that this is actually a compact metric space.
This follows from the fact that $h$ is continuous on $[-\infty,\infty]$ and its range is $[-1,1]$.
...
Let $X=C[-1,1]$ be the space of continuous functions that map $[-1,1]$ to $\mathbb{R}$ and note that $\left\| f \right\|_2 < \infty$, where $\left\| f \right\|_2$ denotes the $L^2$ norm
\begin{equation*}
...
For Cauchy sequence equivalence classes $[q_n]$ and $[r_n]$ with real limits $q$ and $r$, the metrics satisfy
\[d_{\mathcal{C}(\mathbb Q)}([q_n],[r_n])=\lim_{n\to\infty}|q_n-r_n|=|q-r|=d_{\RealNumbers}(q,r). \]
\end{example}
...
\begin{definition} \label{definition:metric_compact}
A metric space is \defn{metric space}{compact} if it is complete~and~totally~bounded.
\end{definition}

For example, the closed interval $[0,1]\subset \RealNumbers$ is compact.
...
\begin{remark}
For general topology, compactness is described by Definition~\ref{definition:topology_compact}.
\end{remark}
...
This follows from the fact that $[0,1]$ is uncountable.
In constrast, pointwise convergence of real functions on a countable domain is metrizable.
\end{example}
...
\begin{definition} \label{definition:topology_compact}
A topological space $X$ is \defn{topology}{compact} if every covering of $X$ by open sets contains a finite subcover.
\end{definition}

\begin{remark}
For metric spaces, this is equivalent to the earlier Definition~\ref{definition:metric_compact}.
\end{remark}
