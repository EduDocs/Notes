...
\begin{definition}
For any metric space $(X,d)$ and subset $W \subseteq X$, a point $w\in X$ is a \defn{metric space}{limit point} of $W$ if there is a sequence $w_1,w_2,\ldots\in W$ of distinct elements that converges to $w$.
\end{definition}
...
Finally, the \defn{metric space}{boundary} $\partial A$ of a set $A$ is defined by $\partial A \triangleq \Closure{A} \setminus \Interior{A}$.
