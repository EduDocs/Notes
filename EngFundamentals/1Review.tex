...
\begin{example}
Let $X$ be the space of continuous functions that map $[-1,1]$ to $\mathbb{R}$ and satisfy $\left\| f \right\|_2 < \infty$, where $\left\| f \right\|_2$ denotes the $L^2$ norm
\begin{equation*}
...
\end{array} \right\}
\end{equation*}
It is easy to verify that $d(f_n, f_m) = (m-n)^2/(6m^2n)$ for $m\geq n$.
From this, one can show that the sequence is Cauchy.
But, it does not converge to a continuous function in $X$.
\end{example}
...
In this case, we can choose $n_1 < n_2 < \cdots$ so that $d(x,x_{n_j}) \! < \! \frac{1}{j}$.
Thus, the subsequence converges to $x$.
