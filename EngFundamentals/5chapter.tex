...
x_1 & = \frac{1}{1} (1 - 1\cdot 3 - 1\cdot (-8) ) = 6. 
\end{align*}
\end{example}
The computational complexity of each substitution is roughly $\frac{1}{2} n^2$ operations.

\begin{problem}
Show that set of upper triangular matrices is a subalgebra of the algebra of all square matrices.
Since it is clearly a subspace, one need only verify closure; namely, that the product of two upper triangular matrices is upper triangular.
Explain why this also implies that the set of invertible upper triangular matrices forms a group under matrix multiplication.
\end{problem}
...
Mathematically, it is the unique function mapping matrices to scalars that is (i) linear in each column, (ii) changes sign when two columns are interchanged, and (iii) satisfies $\det(I) = 1$.
...
Any square matrix $A \in F^{n \times n}$ can be factored with $PA = LU$, where the pivot matrix $P$ permutes the rows of $A$ to avoid dividing by 0, $L$ is a unit lower-triangular matrix, and $U$ is an upper-triangular matrix.
If Gaussian elimination encounters no zero pivot (equivalently, for nonsingular $A$, all leading principal minors are nonzero), then we have $A=LU$ with $L$ unit lower triangular and $U$ upper triangular. 
...
Since $\det(A) = \det(PLU) = \det(P^{-1}) \det(L) \det(U) = \det(L) \det(U)$, the problem is reduced to computing the determinant of triangular matrices.
...
Then, adding $\alpha$ times the $j$-th row to the $i$-th row is an \textbf{elementary row-addition operation}.
...
[ L ]_{ij} =
\begin{cases}
\frac{a^{(j)}_{i,j}}{a^{(j)}_{j,j}} \textrm{  if  } 1 \leq j < i \leq n \\
1 \textrm{  if  } i=j \\
0 \textrm{  otherwise}.
\end{cases}
\end{equation*}
...
& \stackrel{(a)}{=} \prod_{j=1}^{n-1}\prod_{i=j+1}^n \left( I - \frac{a^{(j)}_{i,j}}{a^{(j)}_{j,j}} E_{i,j} \right)^{-1} \\
& \stackrel{(b)}{=} \prod_{j=1}^{n-1}\prod_{i=j+1}^n \left( I + \frac{a^{(j)}_{i,j}}{a^{(j)}_{j,j}} E_{i,j} \right) \\
& = I + \sum_{j=1}^{n-1}\sum_{i=j+1}^n \frac{a^{(j)}_{i,j}}{a^{(j)}_{j,j}} E_{i,j},
\end{align*}
...
\[ \widetilde{M} = Q M Q^{-1} = I + \sum_{j=1}^k \sum_{i=j+1}^n \alpha_{ij} Q E_{ij} Q^{-1}, \]
with $Q^{-1} = Q$.
...
If the matrix $A \in \mathbb{C}^{n \times n}$ is Hermitian and admits elimination without a zero pivot, then the LU decomposition allows the factorization $A = L D L^H$, where $L$ is unit lower triangular and $D$ is diagonal.
However, general Hermitian matrices require symmetric pivoting and may require both $1\times1$ and $2\times2$ diagonal blocks.
...
If $A$ is positive definite, then unpivoted factorization always succeeds and the diagonal elements of $D$ are positive and we can write $A = \left(L D^{1/2} \right) \left(L D^{1/2}\right)^H$.
...
To see this, we will describe the LDLT decomposition assuming no zero pivot and use the notation from LU decomposition starting from $A^{(1)} = A$.
...
This results in orthonormal vectors $\{ \vecnot{q}_1,\ldots,\vecnot{q}_l \}$, where $l=\Rank(A)$, such that
...
U = (I + jR) (I - jR)^{-1},
\end{equation*}
we see that a Hermitian matrix $R$ is mapped to a unitary matrix.
