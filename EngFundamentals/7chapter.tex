...
\gpt{Chapter-level feedback: SVD is an essential culmination for ECE students, but this short chapter currently stops just before its most illuminating consequences.  After the four fundamental subspaces, consider adding the variational characterization $\sigma_1=\max_{\|x\|=1}\|Ax\|$, the Eckart--Young--Mirsky best rank-$k$ approximation theorem, and a brief discussion of conditioning.  For the quantum-computing audience, a short section identifying the Schmidt decomposition of a bipartite pure state as an SVD would connect the linear algebra directly to entanglement.  The polar decomposition would also tie SVD back to projection and least squares.  Move the Hermitian spectral theorem to the preceding Canonical Forms chapter, then let this chapter use it as the clean starting point for SVD.}
...
\gpt{Missing hypothesis: this statement is valid for complex square matrices; a real matrix need not have a real Schur form that is triangular (the real Schur form is only block upper triangular).  Replace ``For any square matrix $A$'' with ``For any $A\in\mathbb C^{n\times n}$''.}
\begin{equation*}
U^H A U = T
\end{equation*}
...
\gpt{In the displayed matrix below, ``$\vecnot s_{2,n}$'' should be the scalar ``$s_{2,n}$''.}
\begin{equation*}
...
= \Sigma ,
\end{equation*}
\gpt{Dimension issue: when $m\ne n$, $\Sigma$ is rectangular, so writing it as the diagonal of all $n$ eigenvalues is ambiguous (and impossible if the notation creates an $n\times n$ matrix).  Suggested replacement: ``Hence $U^HAV=\Sigma\in\mathbb R^{m\times n}$, where $\Sigma_{ii}=\sqrt{\lambda_i}$ for $1\le i\le p=\min(m,n)$ and all off-diagonal entries are zero.''  It would also clarify line 159 to say that the first $r$ vectors span $\mathcal R(A)$, so every orthonormal completion lies in $\mathcal R(A)^\bot=\mathcal N(A^H)$ and therefore has $\vecnot u_i^HA=0$.}
as desired.
\end{proof}

This proof gives a recipe for computing the SVD of an arbitrary matrix.
\gpt{Numerical caveat: forming $A^HA$ squares the condition number and can lose the smallest singular values, so this is not the usual stable recipe for computing an SVD.  Suggested replacement: ``This proof gives a conceptual way to derive the SVD from a Hermitian eigenvalue problem.  In numerical work one typically uses bidiagonalization-based SVD algorithms rather than explicitly forming $A^HA$.''}
Consider the matrix 
...
\gpt{Line above: the compact SVD should be ``$A=U_1\Sigma_1V_1^H$''; the Hermitian transpose is missing.}
...
\gpt{Mathematical error: the set of vectors mapped to nonzero vectors is $X\setminus\mathcal N(A)$, not $\mathcal N(A)^\bot$, and it is not a subspace.  Suggested replacement: ``Then the columns of $V_1$ form an orthonormal basis for $\mathcal N(A)^\bot=\mathcal R(A^H)$, while the columns of $V_2$ form an orthonormal basis for $\mathcal N(A)$.''}
...
\gpt{Line above: ``form a orthonormal basis'' should be ``form an orthonormal basis'', and ``form orthonormal basis'' should be ``form an orthonormal basis''.}
...
\gpt{Line above: $t$ is undefined, and the formula using nonzero singular values is intended for $1\le i\le r$.  Replace both upper limits ``$t$'' with ``$r$'', or introduce $t\le r$.}
...
\gpt{Line above: ``$\vecnot v$'' should be ``$\vecnot v_i$''.}
...
\gpt{Omission: students may infer that the singular vectors are unique.  Suggested addition: ``The singular values are uniquely determined.  Singular vectors associated with a repeated singular value are not unique: any unitary change of basis within the corresponding singular subspaces gives another SVD.  Zero-singular-value bases are likewise nonunique.''}
