...
For any square complex matrix $A \in \mathbb C^{n\times n}$, there exists a unitary matrix $U$ such that
\begin{equation*}
U^H A U = T
\end{equation*}
...
\begin{equation*}
A_{n-1} = \begin{bmatrix} s_{2,2} & \cdots & s_{2,n} \\
\vdots & \ddots & \vdots \\
...
The first $r$ vectors span $\mathcal R(A)$, so every orthonormal completion lies in $\mathcal R(A)^\bot=\mathcal N(A^H)$ and therefore has $\vecnot u_i^HA=0$
Let
\begin{equation*}
...
Hence, $U^HAV=\Sigma\in\mathbb R^{m\times n}$, where $\Sigma_{ii}=\sqrt{\lambda_i}$ for $1\le i\le p=\min(m,n)$ and all off-diagonal entries are zero.
...
While the singular values of $A$ are uniquely determined, the singular vectors associated with a repeated singular value are not unique: any unitary change of basis within the corresponding singular subspaces gives another SVD.
For example, the singular vectors with singular value zero can be any basis of the null space.
\end{proof}

This proof gives a conceptual way to derive the SVD from a Hermitian eigenvalue problem. 
In numerical work one typically reduces first to a bidiagonal form rather than explicitly forming $A^HA$.
...
The factorization $A = U \Sigma V^H$ is called the \textbf{full SVD} of the matrix $A$ while the factorization $A = U_1 \Sigma_1 V_1^H$ is called the \defn{matrix}{compact SVD} of $A$.
...
Then the columns of $V_1$ form an orthonormal basis for $\mathcal N(A)^\bot=\mathcal R(A^H)$, while the columns of $V_2$ form an orthonormal basis for $\mathcal N(A)$.
Likewise, the columns of $U_1$ form an orthonormal basis for the vectors in $Y$ that lie in the range of $A$ while the vectors in $U_2$ form an orthonormal basis for $\mathcal{R}(A)^\bot$.
It follows that the full SVD computes orthonormal bases for all four fundamental subspaces of the matrix $A$.
...
\end{align*}
To see this, notice that $A \sum_{i=1}^r c_i \vecnot{v}_i = \sum_{i=1}^r c_i \sigma_i \vecnot{u}_i$.
...
where $\vecnot{u}_i$ is the $i$th column of $U$ and $\vecnot{v}_i$ is the $i$th column of $V$.
