...
\item A formal deductive system consists of a formal language, axioms, and inference rules used to derive theorems. 
%\begin{itemize}
%%\item Argument is \textbf{valid} if conclusion follows from the premie. 
...
\item \textbf{Predicate logic}  (a.k.a.\ first-order predicate logic)  constructs logical statements from a primitive collection of statements \emph{with additional structure}
\end{itemize}

\item \textbf{Remark:} First-order predicate logic is complete in the sense that every logically valid formula is provable, and validity is semidecidable but not decidable. 
\begin{itemize}
\item  \textbf{Consistent \& Complete:}   An argument if valid if and only if it can be proved from axioms
\item \textbf{Decidable:}  True if an argument can be determined in finite time. 
\item  \textbf{Semidecidable:} algorithm determines the truth of any postulated implication, if it terminates. But, termination is guaranteed only if postulate is true
%\end{itemize}
%
...
\item ``Wash your hands before dinner''  -- NO, this is a command
\item ``If you wash your hands, then you won't get sick'' -- YES
...
% \gpt{Mathematical precision: the law of excluded middle is the assertion $P\vee\neg P$.  The claim that every proposition has exactly one of the truth values true and false is the principle of bivalence; in classical logic it implies that ``not false'' means true.}
...
\item Combining multiple operations results in \textbf{propositional formula} or \textbf{compound formula} 
% \gpt{Typos/terminology: replace ``propositional formual'' by ``propositional formula'' and ``compound operation'' by ``compound proposition'' (or ``compound formula'').}
\[
...
\subsection{Relations} 
   ...
   \begin{itemize}
\item  \textbf{Definition:}  A \textbf{relation} (or meta statement) is a logical relation between propositional formulas (i.e., compound operations). % compound operations on logical statements (i.e. propositional formulas).  %(While the output of operation, the output of a relation is not a logical statement). 
% \gpt{Standard terminology: rename this subsection ``Logical Equivalence and Logical Consequence.''  $R$ and $S$ are formulas, not operations.  If every truth assignment satisfying $R$ also satisfies $S$, standard notation is semantic entailment $R\models S$; if they have the same truth value under every assignment, write semantic equivalence $R\equiv S$.  This also distinguishes these meta-level relations from the object-language connectives $\to$ and $\leftrightarrow$.}
...
P \to Q \iff  \neg Q \to\neg P 
\]
i.e., ``if $P$ then $Q$''  is equivalent to ``if not $Q$ then  not $P$'''
\end{itemize}
...
\item $\neg (P \to Q )  \implies  Q$ ?
\end{itemize}
% \gpt{The first two implications above are valid, but the third is false: $\neg(P\to Q)\equiv P\wedge\neg Q$, so it implies $\neg Q$, not $Q$.  Suggested replacement for the third line: $\neg(P\to Q)\implies\neg Q$.}
...
\end{itemize}
% \gpt{Terminology: tautology and contradiction classify formulas, not operations.  A tautology is valid under every truth assignment; a contradiction is unsatisfiable.  A formula true under some assignments and false under others is commonly called a contingency.  Introducing ``satisfiable'' here would also prepare students for later logic terminology.}
...
% \gpt{Terminology: $U$ is usually called the ``domain of discourse'' (or ``universe of discourse'').  ``Universal set'' is more common in elementary set theory and can misleadingly suggest a set containing every mathematical object.}
\item predicate $P(x)$ is a statement that can be applied to any $x \in U$
\item $x$ is called a free variable 
% \gpt{Terminology omission: after applying $\forall x$ or $\exists x$, the variable $x$ is \textbf{bound}.  A formula with no free variables is a \textbf{sentence} (or closed formula), and only a sentence has a truth value without first assigning values to variables.}
\end{itemize}


\item \textbf{Quantifies} allow statements about collections of elements
% \gpt{Typos in the introductory lines above: replace ``allows allows'' by ``allows,'' ``predicate $P(x)$ ia statement can can be applied'' by ``the predicate $P(x)$ is a statement that can be applied,'' and ``Quantifies'' by ``Quantifiers.''}
\begin{itemize}
...
\end{itemize}
% \gpt{Standard terminology: the universal statement over the empty set is \textbf{vacuously true}; call this phenomenon ``vacuous truth.''  This phrase is used routinely in analysis, especially when checking definitions over empty collections.}
...
% \gpt{Typos/notation: replace ``people in the word'' by ``people in the world,'' and replace $\forall x\in X$ by $\forall x\in S$ because $S$ is the set of people over ten feet tall and $X$ is undefined here.}
...
% \gpt{Grammar: replace ``Does the following implication holds?'' by ``Does the following implication hold?''; below, write ``nonempty'' and ``the empty set.''}
\[
...
\end{align*}
% \gpt{Two quantified formulas above are malformed.  In the first line, commute the quantifiers on the right: $\forall y\,\forall x\,P(x,y)$.  In the third line, use $\exists x\,\forall y\,P(x,y)$ on the right; the current expression binds $x$ twice and leaves $y$ free.}
...
\end{itemize}
% \gpt{Standard proof terminology: use ``proof by contraposition'' rather than ``contrapositive'' for the method.  For mathematical induction, name the \textbf{base case}, \textbf{induction hypothesis} (assume $P(n)$ for an arbitrary $n$), and \textbf{inductive step} (deduce $P(n+1)$).  Avoid reusing $P$ and $Q$ for both predicates and whole propositions.}
...
% \gpt{Mathematical/wording correction: because $x^2=2$ permits negative $x$, define rationality using $p\in\mathbb Z$ and $q\in\mathbb Z\setminus\{0\}$ (or $q\in\mathbb N$ with $p\in\mathbb Z$).  Also replace ``Goal it so show'' by ``The goal is to show.''}
...
% \gpt{Terminology/precision: say ``write $x=p/q$ in lowest terms, so $\gcd(p,q)=1$'' or ``$p$ and $q$ are relatively prime.''  Coprime integers do share the divisor $1$, so the parenthetical should be ``no common divisor greater than $1$.''}
\begin{align*}
...
%\quad \implies \quad (2k)^2   = 2 q^2 \quad \implies \quad 2k^2 = q^2
  %\quad  \implies  \quad \text{$p^2$ is even} 
   %\quad  \implies  \quad \text{$p$ is even} \wedge \text{$q$ is even}
\end{align*}
% \gpt{Proof omission in the two chains above: both use the converse ``if an integer's square is even, then the integer is even.''  State or prove this lemma first (for example, by the contrapositive: the square of an odd integer is odd).}
...
\end{align*}
% \gpt{TeX typo: remove the stray final backslash after the last parenthesis in the preceding aligned display.}
...
% \gpt{Grammar: replace ``Then shows'' by ``Then show'' and ``where second equality follow'' by ``where the second equality follows.''}
\end{itemize} 
...
\]
% \gpt{Mathematical/typographical error: the first displayed set omits $u$, so the asserted equality is false.  Suggested replacement: $A=\{i,o,u,a,e\}=\{a,e,i,o,u,e,o\}$.  Also replace ``have not effect'' by ``have no effect.''}
...
\[ x\in A-B \iff (x\in A)\wedge (x\notin B) \]
  ...
  \[ x\in A^c \iff x \notin A \]
% \gpt{Terminology/notation: $A\setminus B$ is the standard notation for \textbf{set difference} (also called the relative complement of $B$ in $A$); $A-B$ can be confused with arithmetic subtraction.  The notation $A^c$ denotes the complement of $A$ relative to a specified ambient set $U$.}
\end{itemize}
...
% \gpt{Notation convention: $\subset$ is ambiguous across texts---some authors allow equality.  Use $A\subsetneq B$ for ``proper subset'' and reserve $A\subseteq B$ for arbitrary inclusion.}
...
% \gpt{Notation: logical formulas are equivalent rather than literally equal; replace $\neg(P\vee Q)=\neg P\wedge\neg Q$ by $\neg(P\vee Q)\iff\neg P\wedge\neg Q$.}
\begin{align*}
...
% \gpt{Terminology: call these \textbf{indexed} or \textbf{arbitrary unions and intersections}, not infinite unions and intersections, because the index set $I$ may be finite or empty.  The family $(S_\alpha)_{\alpha\in I}$ is an indexed family of sets.}
  \begin{align*}
...
Then:
  ...
  \end{align*}  
% \gpt{Mathematical error: as defined, every $A_n=\mathbb N$, so both the union and intersection equal $\mathbb N$.  To obtain the stated union/intersection, define $A_n=\{x\in\mathbb N:x\ge n\}$; then $\bigcup_nA_n=\mathbb N$ and $\bigcap_nA_n=\emptyset$.}
...
% \gpt{Typo: replace ``De Morgans'' by ``De Morgan's law.''}
\[
...
% \gpt{Grammar: replace ``sometime'' by ``sometimes.''}
...
% \gpt{Standard terminology: this result is the \textbf{Cantor--Schröder--Bernstein theorem}.  Its conclusion is that there exists a \textbf{bijection} $f\colon A\to B$; ``one-to-one invertible mapping'' and the arrow $A\leftrightarrow B$ are nonstandard here.}
...
% \gpt{Two errors above: the first inequality should be $|\mathbb N|\leq|\mathbb N^2|$, and the diagonal enumeration repeats $(3,1)$.  The sixth pair should be $(1,3)$, giving $(1,1),(2,1),(1,2),(3,1),(2,2),(1,3),\ldots$.}
%\vspace{.2in} 
%\end{center} 
...
% \gpt{Proof omission: explain the injection $\mathbb Q\hookrightarrow\mathbb N^2$, since signed numerators and nonunique fraction representations must be handled.  One option is to use the unique reduced representation $p/q$ with $p\in\mathbb Z$, $q\in\mathbb N$, $q>0$, and encode the signed integer $p$ as a natural number.}
...
% \gpt{Typos/grammar: replace ``Termonology'' by ``Terminology'' and ``mean finite or countably infinity'' by ``means finite or countably infinite.''}

\item \textbf{uncountable} means cardinality strictly greater than $\bbN$. 
% \gpt{Terminology: define \textbf{countable} as finite or in bijection with $\mathbb N$ (also ``at most countable''), \textbf{countably infinite} as in bijection with $\mathbb N$, and \textbf{uncountable} simply as ``not countable.''  ``Enumerable'' is sometimes used for countable, while ``denumerable'' specifically means countably infinite; because usage varies, state the convention explicitly.}
%
...
\begin{itemize}
\item Set-builder notation is safe when it selects elements from a specified set $X$.  If one instead allows \textbf{unrestricted comprehension}, then one is allowing every property $P(S)$ to define a set $\{S \in \mathbb{S} \mid P(S)\}$, where $\mathbb{S}$ represents the set of all sets.

\item For $P(S) =$ ``Set $S$ does not contain itself'', this gives the set
\[
R \coloneqq \{S \in \mathbb{S} \mid S \notin S\}.
\]
Set-builder notation implies that $S\in R$ if and only if $S \notin S$.
Evaluating this statement with $S=R$ gives
\[
R \in R \;\leftrightarrow\; R \notin R,
\]
which is a contradiction.  This is \textbf{Russell's paradox}.

\item Therefore, $R$ cannot exist in any consistent theory of sets.  Axiomatic set theory avoids the paradox by restricting set formation. In this course, we use standard set constructions with explicit ambient sets and this avoids such contradictions.
\end{itemize}
...
% \gpt{Grammar/notation: replace ``parentheses denotes'' by ``parentheses denote,'' and write the final product as $A\times\cdots\times A$ (with $n$ factors).}
...
% \gpt{Terminology: this is a \textbf{binary relation on $A$}.  More generally, a binary relation \emph{from $A$ to $B$} is a subset of $A\times B$.  Saying ``on $A$'' signals that both coordinates come from the same set.}
...
\end{itemize}
% \gpt{Typo in the reflexive bullet: remove the unmatched closing parenthesis after $R$.}
...
% \gpt{Standard classification: $\geq$ on $\mathbb R$ is a \textbf{total order} (with the orientation opposite to $\leq$).  In addition to being reflexive and transitive, it is antisymmetric and total/connex.  Introducing ``partial order'' and ``total order'' avoids presenting this merely as a failed equivalence relation.}
...
% \gpt{Grammar: write ``$x$ has the same birthday as $y$'' and ``This is an equivalence relation.''}
...
% \gpt{Terminology: the members of a partition are conventionally called its \textbf{blocks}, \textbf{parts}, or \textbf{cells}.  Thus, say that every element of $A$ lies in exactly one block of $\mathcal P$.}
...
\emptyset \notin \in \cP, \qquad  \bigcup_{X  \in \cP } X  = A , \qquad    \forall X, Y \in \cP, \; X \ne Y  \implies A \cap B = \emptyset
 \]
% \gpt{The partition sentence repeats ``such that,'' and the display has two mathematical typos.  Suggested replacement: $\emptyset\notin\mathcal P$, $\bigcup_{X\in\mathcal P}X=A$, and $\forall X,Y\in\mathcal P,\ X\neq Y\Rightarrow X\cap Y=\emptyset$.  The current display has the invalid $\notin\in$ and uses undefined $A\cap B$ instead of $X\cap Y$.}
...
% \gpt{Omission: specify the index set and exclude empty date classes, since impossible dates (and possibly February 29 in a particular population) give empty $A_{ij}$, while partition blocks must be nonempty.  For example, use $\mathcal P=\{A_{ij}:A_{ij}\neq\emptyset\}$.}
...
\]
% \gpt{Notation error: the quotient by an equivalence relation is conventionally $A/\!\sim$, not $A\setminus\sim$.  The later birthday example already uses $A/\sim$.}
...
% \gpt{Typos: replace ``be equivalence relation defied set'' by ``be the equivalence relation defined by'' and insert ``the'' in ``has the same birthday.''}
\[
[a]  = A_{i,j} 
\]
...
% \gpt{Standard terminology: rename this ``Congruence modulo $p$.''  Write $x\equiv y\pmod p$ when $p\mid(x-y)$; the equivalence classes are \textbf{residue classes modulo $p$}, and the quotient is denoted $\mathbb Z/p\mathbb Z$.  Avoid $\mathbb Z_p$ unless its meaning is declared, because that notation can instead denote the $p$-adic integers.}
\begin{itemize}
...
\]
% \gpt{Terminology/precision: if $r$ is called a remainder, require $0\leq r<p$.  Alternatively, define congruence more directly by $x\sim y\iff p\mid(x-y)$; the current unrestricted integer $r$ describes congruence but not a unique division remainder.}
...
% \gpt{Typos/grammar in this paragraph: replace ``classess'' by ``classes,'' ``classes has'' by ``classes have,'' and ``denote'' by ``denotes.''  More precisely, this is additive coset structure rather than vector-space linear structure.}
\end{itemize}
...
\end{itemize}
% \gpt{Terminology: many modern texts call $f(X)$ the \textbf{image} of $f$ and reserve \textbf{codomain} for the declared target $Y$.  ``Range'' is used inconsistently for either one, so explicitly declaring ``range/image $f(X)$'' will prevent ambiguity.}
...
% \gpt{Typos/notation: replace ``Defintion'' by ``Definition,'' ``from set $X$ to set $Y$'' by ``from a set $X$ to a set $Y$,'' and preferably use $F\subseteq X\times Y$ because equality is allowed (for example, when $X$ is a singleton and $Y$ contains only its assigned value).}
% \gpt{Standard terminology: the subset $F=\{(x,f(x)):x\in X\}$ is the \textbf{graph} of $f$.  For a general map, $f^{-1}(\{y\})$ is the \textbf{fiber} (or level set) over $y$; here the singleton attached to each $x$ is better described directly as the unique output rather than as $A_x$.}
...
% \gpt{Wording: replace ``value of all elements'' by ``the same value at every element of the domain.''}
...
% \gpt{This paragraph has an incomplete opening sentence and omits measurability.  Suggested replacement: ``Sometimes it is useful to define an equivalence relation on a set of functions.  The space $L^2([0,1])$ consists of equivalence classes of Lebesgue-measurable functions with finite $\int_0^1|f|^2$, where $f\sim g$ iff $\int_0^1|f-g|^2=0$, equivalently $f=g$ almost everywhere.''  Also replace ``two function ... event'' by ``two functions ... even.''  They may differ on any measure-zero set, not only a countable one.}
...
\end{itemize}
% \gpt{Mathematical omission: invertibility depends on both domain and codomain.  The first answer is YES only for $f\colon[0,\infty)\to[0,\infty)$.  For $f\colon[0,\infty)\to\mathbb R$, the map is not onto and hence is not invertible.}
...
% \gpt{Terminology: replacing the codomain $Y$ by the image $R=f(X)$ is called \textbf{corestricting} $f$ to its image.  Thus every injection becomes a bijection after corestriction to its image.}
 \end{itemize}
...
% \gpt{Terminology: the following image and preimage operations are induced set maps $f(\cdot)\colon\mathcal P(X)\to\mathcal P(Y)$ and $f^{-1}(\cdot)\colon\mathcal P(Y)\to\mathcal P(X)$, not ``set-valued functions'' in the usual sense of a map that assigns a set to each point.  Consider renaming the subsection ``Images and Preimages of Sets.''}
 \begin{itemize}

\item \textbf{Definition:} For $f \colon X \to Y$ and subset $A \subseteq X$, the \textbf{image} of $A$ under of $f$ is the set
% \gpt{Typo: replace ``under of $f$'' by ``under $f$.''}
\[
...
\item \textbf{Definition:} For $f \colon X \to Y$ and subset $C \subseteq Y$, the \textbf{inverse image} or \textbf{preimage} of $C$ is the set
\[
...
f^{-1}( f(A)) \supseteq A  \quad \text{and} \quad f( f^{-1}(B)) \subseteq B
\]
% \gpt{Useful strengthening/omission: $f(f^{-1}(B))=B\cap f(X)$ for every $B\subseteq Y$.  Also, $f^{-1}(f(A))=A$ for every $A\subseteq X$ iff $f$ is injective.}


\item \textbf{Question:} For $f \colon X \to Y$ and collection of subsets $A_i \subseteq X$ for $i \in I$, 
does the following identity hold? 
% \gpt{Notation/grammar: replace $A_i\in X$ by $A_i\subseteq X$, and replace ``does it hold that?'' by ``does the following identity hold?''}
\[
...
\[
y \in f\left(\bigcup_{i \in I} A_i \right)  \iff y  \in  \bigcup_{i \in I} f(A_i) 
\]

\item Proof of $\Longleftrightarrow$
\begin{align*}
y \in f\left(\bigcup_{i \in I} A_i \right) & \Longleftrightarrow  \exists  x  \in \bigcup_{i \in I} A_i, \, f(x)  = y\\
& \Longleftrightarrow \exists i \in I, \, \exists x \in A_i, \,  f(x) = y \\
& \Longleftrightarrow \exists i \in I, \,  y \in f(A_i) \\
