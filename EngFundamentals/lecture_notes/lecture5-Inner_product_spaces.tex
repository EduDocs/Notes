...
% \gpt{Suggested replacement: ``A Banach space is separable if it has a Schauder basis, but the converse fails in general. Every separable Hilbert space does have a countable orthonormal basis.''}
...
\end{equation*}
% \gpt{Suggested cut/paste qualification: if completeness is needed later, take $V=L^2([0,1])$ modulo equality almost everywhere. The continuous-function subspace has this inner product but is not complete in the induced norm.}
...
\[
 ...
 \]
% \gpt{Suggested cut/paste hypothesis: require $\vecnot{v}\neq\vecnot{0}$ before defining projection onto $\vecnot{v}$.}
...
\end{alignat*}
% \gpt{Suggested cut/paste correction throughout this display: replace the unintended $\vecnot{v}$ by $\vecnot{u}$ in both inner products and in the Cauchy--Schwarz bound.}
