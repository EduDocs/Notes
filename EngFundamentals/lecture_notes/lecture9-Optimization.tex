...
\]
% \gpt{Suggested cut/paste hypothesis: work in a normed vector space and require distinct $a_1,a_2\in\overline A$ in the strict-convexity clause; closure and interior require a topology.}
\begin{center}
...
% \gpt{Suggested cut/paste qualification: require $A$ to be nonempty as well as closed and convex. The revised book also records the nonexpansive bound $\|P_A(v)-P_A(w)\|\leq\|v-w\|$.}
...
\]
% \gpt{Suggested cut/paste correction: the numerator must evaluate along the segment from $x_0$ to $x$, namely $f(x_0+\lambda(x-x_0))-f(x_0)$.}
...
% \gpt{Suggested constrained optimality condition: ``If $f'(x_0;x-x_0)\geq0$ for every $x\in A$, then $x_0$ is a global minimizer; if $f$ is strictly convex, it is unique.'' Requiring zero in all directions is unnecessarily strong and may include infeasible directions.}
