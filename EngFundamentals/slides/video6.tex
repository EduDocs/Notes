...
\note{
	...
	\end{enumerate}
	\gpt{The displayed metric uses $x/(1+|x|)$ at $x=\pm\infty$, where the formula is otherwise undefined.  Consider explicitly defining the embedding value at the endpoints as $h(+\infty)=1$ and $h(-\infty)=-1$, then writing the metric as $|h(x)-h(y)|$.}
}
...
Conversely, if $f(x_n) \to f(x_0)$ for any sequence $x_1,x_2,\ldots  \in X$ such that $x_n \to x_0$, then $f$ is continuous at $x_0$.
\end{theorem}
...
\note{
	...
	\end{enumerate}
	\gpt{Grammar on overlay 3: replace ``It is uniformly continuous?'' with ``Is it uniformly continuous?''}
}
...
\note{
	...
	\end{enumerate}
	\gpt{The last sentence of cue 3 should not say that the first derivative ``is infinite'' at zero.  The derivative is not finite at the endpoint and is unbounded as $x\downarrow0$.  Likewise, the displayed supremum is infinite; no individual finite-$z$ expression equals infinity.}
}
...
\note{
	...
	\end{enumerate}
	\gpt{Please verify the section references: this deck labels continuity as 2.1.2 and real-number properties as 2.1.4, while the suggested reading is 2.1.3--2.1.5.  Also, this source contains dormant frames after the first \texttt{\string\end\{document\}}; the current extraction diagnostic counts those frames even though the rendered production deck stops at 24 pages.}
}
