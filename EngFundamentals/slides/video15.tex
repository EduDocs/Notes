...
\end{equation*}
\end{example}
% \gpt{Suggested cut/paste qualification: if this example is meant to introduce the Hilbert space used later, take $V=L^2([0,1])$ modulo equality almost everywhere.  The continuous-function subspace has this inner product but is not complete in the induced norm.}
...
\end{example}
% \gpt{Suggested cut/paste qualification: identify random variables that are equal almost surely; otherwise $\mathbb{E}[X^2]=0$ need not imply that $X$ is the zero pointwise function.}
...
\end{tabular}
\end{definition}
% \gpt{Suggested cut/paste hypothesis: require $\vecnot{v}\neq\vecnot{0}$ before defining projection onto $\vecnot{v}$, since the formula divides by $\|\vecnot{v}\|^2$.}
...
\end{enumerate}
\end{theorem}
% \gpt{Suggested cut/paste equality cases: Cauchy--Schwarz has equality iff the vectors are linearly dependent; triangle inequality has equality iff one vector is a nonnegative real multiple of the other (including the zero cases).  An arbitrary complex or negative scalar is too broad for the triangle equality case.}
