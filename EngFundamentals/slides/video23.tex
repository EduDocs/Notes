...
\end{lemma}
% \gpt{Suggested cut/paste correction: ``The dual optimal value is $d^*=\sup_{\lambda\geq0,\nu}g(\lambda,\nu)$ and satisfies $d^*\leq p^*$.''  Concavity does not imply a unique maximizer, and the supremum need not be attained.}
...
% \gpt{Suggested replacement proof: for any primal-feasible $x'$ and any $\lambda\geq0$, $g(\lambda,\nu)=\inf_{x\in\mathcal D}L(x,\lambda,\nu)\leq L(x',\lambda,\nu)\leq f_0(x')$.  Taking the infimum over feasible $x'$ and then the supremum over dual-feasible multipliers gives $d^*\leq p^*$.  This avoids the invalid step that pulls an unspecified feasible $x$ out of an infimum.}
