...
\end{definition}
% \gpt{Suggested cut/paste hypothesis: make $X$ a normed (or Banach) space before requiring $h\mapsto\delta f(x;h)$ to be continuous; continuity is not defined on a bare vector space.}
...
\end{example}
% \gpt{Suggested cut/paste scope note from the August 23 book: ``For arbitrary Hilbert space $X$, this step uses the general Riesz representation theorem; the earlier finite-dimensional Riesz theorem alone is not sufficient.  Cite the general theorem here, or restrict this example to finite-dimensional $X$.''}
...
\end{lemma}
% \gpt{Suggested cut/paste hypothesis from the revised book: choose a convex set $A\subseteq X$, assume the Lipschitz-gradient bound for every $x,y\in A$, and state the conclusion for $x,y\in A$; convexity keeps the line segment used in the proof inside the domain.}
\vspace{-1mm}
\begin{proof}<2->
