...
\end{definition}
% \gpt{Suggested cut/paste hypothesis: work in a normed vector space and require distinct $a_1,a_2\in\overline A$ in the strict-convexity clause; the closure/interior notation needs a topology, and equal endpoints cannot yield an interior point in general.}
\end{minipage}
\hfill
...
\end{corollary}
% \gpt{Suggested cut/paste constrained optimality condition: ``If $\delta f(x_0;x-x_0)\geq0$ for every $x\in A$, then $x_0$ is a global minimizer of convex $f$ on $A$; if $f$ is strictly convex, it is unique.''  Requiring zero in all directions is unnecessarily strong and may refer to infeasible directions.}
...
% \gpt{Suggested cut/paste hypothesis: require $A$ to be nonempty as well as closed and convex.  The revised book also adds the useful conclusion $\|P_A(v)-P_A(w)\|\leq\|v-w\|$.}
\[
