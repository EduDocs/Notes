...
% \gpt{Suggested cut/paste proof repair: if $v_i\to v$, write $\|Tv_i-Tv\|=\|T(v_i-v)\|\leq M\|v_i-v\|\to0$.  The current Cauchy-sequence argument would require completeness of $W$.}
...
% \gpt{Suggested cut/paste hypothesis from the revised book: ``Let $V$ be a Banach space and $T\in\mathcal{L}(V,V)$ satisfy $\|T\|<1$.''  Completeness is what guarantees convergence of the operator series.}
