...
\IfFileExists{../0Set_Theory_and_Logic.aux}{%
  \externaldocument{../0Set_Theory_and_Logic}%
}{%
  \externaldocument{../auxdir/0Set_Theory_and_Logic}%
}
\makeatletter
% Beamer wraps \ref in an internal slide link.  For labels imported from the
% book, extract their PDF destination and create a remote GoTo link instead.
\newcommand{\bookref}[1]{%
...
% The slide-only PDF is rasterized for video production. Keep external links
% active, but suppress their visible borders in those rendered frames.
...
\begin{frame}<1-7> \frametitle{\bookref{chapter:LogicAndSetTheory}: Logic}
...
\begin{itemize}
  ...
  \item<6-> Ex. ``\univ is in \city \textcolor{red}{or} all real numbers are rational''
...
\begin{frame}<1-4> \frametitle{\bookref{section:Statements}: Basic Definitions}
...
\begin{frame}<1-4> \frametitle{\bookref{section:Relations}: Conditional Statements (1)}
...
\item When $P$ is false, some people guess that the truth value should be undefined. But, the values shown here are \textcolor{blue}{universally accepted} in logic
...
So, if it rains and they don't ride their bike, then most people would agree that they've kept their promise.
...
\begin{frame}<1-4> \frametitle{\bookref{section:Relations}: Conditional Statements (2)}
...
\begin{frame}<1-2> \frametitle{\bookref{section:Relations}: Compound Statements}
...
\begin{frame}<1-5> \frametitle{\bookref{section:Relations}: Meta Statements}
...
\item<4-> The \textcolor{blue}{implication $R \Rightarrow S$} is a meta statement (e.g., for $R(P,Q),S(P,Q)$) \vspace{1mm}
\begin{itemize}
  \setlength\itemsep{1.5mm}
...
\item<5-> The \textcolor{blue}{equivalence $R \Leftrightarrow S$} is a meta statement (e.g., for $R(P,Q),S(P,Q)$) \vspace{1mm}
\begin{itemize}
  \setlength\itemsep{1.5mm}
...
\begin{itemize}
 \setlength\itemsep{3mm}
 \item Try the suggested reading: Course Notes EF \bookref{chapter:LogicAndSetTheory}--\bookref{subsection:Quantifiers}
