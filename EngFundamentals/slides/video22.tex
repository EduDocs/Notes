...
\title{ECE 586: Vector Space Methods \\ Lecture 22: Projection onto Convex Sets}
...
\begin{theorem}<2->
For any $\vecnot{v} \notin A$, we have $\vecnot{u}^* = P_A (\vecnot{v})$ iff $\langle \vecnot{v}-\vecnot{u}^* \,|\, \vecnot{u}-\vecnot{u}^* \rangle \leq 0$ for all $\vecnot{u}\in A$.
\end{theorem}
