...
\begin{frame}<1-9> \frametitle{\bookref{section:SetTheory}: Set Theory}
...
\begin{frame}<1-7> \frametitle{\bookref{section:SetTheory}: Using Set Theory}
...
\begin{frame}<1-5> \frametitle{\bookref{section:SetTheory}: Set Properties}
...
\begin{frame} \frametitle{\bookref{section:SetTheory}: Venn Diagrams}
...
\begin{frame}<1-4> \frametitle{\bookref{section:SetTheory}: From Logic to Set Theory}
...
\[ x\in A-B \Leftrightarrow (x\in A)\wedge (x\notin B) \]
  \item<4-> \textcolor{blue}{Complement} of $A$ (denoted $A^c$) equals $U-A$, for an implied universal set $U\!\!\!\!\!\!\!\!\!\!\!\!\!\!$
...
% \gpt{Suggested new proof-workshop frame from the August 23 book: prove $A\cap(B\cup C)=(A\cap B)\cup(A\cap C)$ element by element.  For arbitrary $x$, write $x\in A\cap(B\cup C)\Leftrightarrow[x\in A\wedge(x\in B\vee x\in C)]\Leftrightarrow[(x\in A\wedge x\in B)\vee(x\in A\wedge x\in C)]\Leftrightarrow x\in(A\cap B)\cup(A\cap C)$.  Since the equivalence holds for every $x$, the sets are equal.}

\begin{frame}<1-4> \frametitle{\bookref{section:SetTheory}: Relationships Between Sets}
...
\begin{frame}<1-3> \frametitle{\bookref{section:SetTheory}: De Morgan, Infinite Operations, and Negation}
...
\begin{frame}{\bookref{section:SetTheory}: Foundations of Set Theory}

\vspace{4mm}

\begin{example}[Russell's Paradox]
Let $R = \{S|S\notin S\}$ be the set of sets that do not include themselves as members.
This set is not forbidden by naive set theory because it is described by the above sentence.
...
\begin{itemize} 
  ...
  \item It is avoided in ZF axiomatic formulation by restricting set comprehension %and explicitly with the theorem: ``no set contains itself''
\end{itemize}
...
\begin{itemize}
 \setlength\itemsep{3mm}
 \item Try the suggested reading: Course Notes EF \bookref{section:SetTheory}
