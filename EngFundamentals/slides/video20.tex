...
%\end{proof}
% \gpt{If the commented Hermitian-eigenvector proof is restored, use the August 23 repair: from $Av=\lambda_1v$, $v^HAv=\lambda_1\|v\|^2$ is real, hence $\lambda_1\in\mathbb R$; if $Aw=\lambda_2w$, then $\lambda_1w^Hv=w^HAv=(Aw)^Hv=\lambda_2w^Hv$, so distinct eigenvalues imply $w^Hv=0$.}
...
% \gpt{Suggested cut/paste correction: the compact SVD is $A=U_1\Sigma_1V_1^H$; the conjugate transpose is missing here.}
% \gpt{Suggested addition from the August 23 book: ``The singular values are uniquely determined, but singular vectors associated with a repeated singular value are not unique.  The zero-singular-value right and left vectors may be any orthonormal bases of $\mathcal N(A)$ and $\mathcal N(A^H)$, respectively.''}
...
% \gpt{Suggested numerical caveat from the August 23 book: ``This $A^HA$ construction is a conceptual proof, not the usual stable numerical algorithm: explicitly forming $A^HA$ squares the condition number and can lose small singular values.  Numerical SVD routines typically begin with bidiagonalization.''}
...
\end{theorem}
% \gpt{Suggested cut/paste theorem statement: minimize over $\operatorname{rank}(B)\leq k$, not only rank exactly $k$.  Also note that repeated singular values can make the singular vectors and best rank-$k$ approximant nonunique.}
