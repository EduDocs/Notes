...
For a given vector $\vecnot{v} \in V$, we seek an element of $W$ that provides a best approximation to $\vecnot{v}$.
As illustrated in Figure~\ref{fig:AR-Banach1}, the fundamental problem is to identify a vector $\vecnot{w} \in W$ that minimizes the distance $\| \vecnot{v} - \vecnot{w} \|$.
...
In this example, the complete vector space $V=\mathbb{R}^3$ is endowed with the $\ell_1$-norm; the subset $W$ corresponds to the $xz$-plane; the vector $\vecnot{v}$ is the point in blue; and its best approximation $\vecnot{w}$ is shown in red.
The octahedron is the unit ball of the $\ell_1$-norm.}
...
&= \inner{ \vecnot{v} }{ \vecnot{w}_j }
- \frac{ \inner{ \vecnot{v} }{ \vecnot{w}_j } }{ \left\| \vecnot{w}_j \right\|^2 } \inner{ \vecnot{w}_j }{ \vecnot{w}_j }
= 0.
\end{split}
\end{equation*}
...
An illustrative diagram of an orthogonal projection appears in Figure~\ref{fig:AR-Projection1}.
...
\begin{corollary}
Let $\vecnot{w}_1, \vecnot{w}_2, \ldots$ be a sequence of non-zero pairwise orthogonal vectors in an inner-product space $V$ and let $W_n = \Span(\vecnot{w}_1, \ldots, \vecnot{w}_n)$.
If $\vecnot{v} \in V$, then
\begin{equation*}
\sum_{i=1}^\infty \frac{ \left| \inner{ \vecnot{v} }{ \vecnot{w}_i } \right|^2 }{ \left\| \vecnot{w}_i \right\|^2 }
\leq \left\| \vecnot{v} \right\|^2.
\end{equation*}
Moreover, equality holds if and only if
\begin{equation*}
\vecnot{v} = \sum_{i=1}^\infty \frac{ \inner{ \vecnot{v} }{ \vecnot{w}_i } }{ \left\| \vecnot{w}_i \right\|^2 } \vecnot{w}_i .
\end{equation*}
\end{corollary}
\begin{proof}
Let the projection of $\vecnot{v}$ onto $W_n$ be
\begin{equation*}
\vecnot{v}_n = \sum_{i=1}^n \frac{ \inner{ \vecnot{v} }{ \vecnot{w}_i } }{ \left\| \vecnot{w}_i \right\|^2 } \vecnot{w}_i  .
\end{equation*}
By construction, the error vector $\vecnot{u}_n = \vecnot{v} - \vecnot{v}_n$ satisfies  $\inner{ \vecnot{u}_n }{ \vecnot{v}_n } = 0$ and, hence, $\left\| \vecnot{u}_n \right\|^2 = \left\| \vecnot{v} \right\|^2 - \left\| \vecnot{v}_n \right\|^2$.
Noting that $\| \vecnot{u}_n \|^2 \geq 0$ and 
\begin{equation*}
\left\| \vecnot{v}_n \right\|^2
= \sum_{i=1}^n \frac{ \left| \inner{ \vecnot{v} }{ \vecnot{w}_i } \right|^2 }{ \left\| \vecnot{w}_i \right\|^2 },
\end{equation*}
we see that $\| \vecnot{v}_n \|^2 \leq \| \vecnot{v} \|^2$ with equality if and only if $\vecnot{u}_n = \vecnot{0}$.
Taking the limit, as $n \to \infty$, of the previous equation yields the first result.
This limit is achieved with equality if and only if $\vecnot{u}_n \to 0$, which is equivalent to the second result.
\end{proof}
...
The range $\mathcal{R}(T)$ and nullspace $\mathcal{N}(T)$ are subspaces of $V$ with trivial intersection: $\mathcal{R}(T)\cap\mathcal{N}(T)=\{\vecnot{0}\}$.
\end{theorem}
\begin{proof}
...
% Do not reiterate this comment
%\gpt{The displayed sums use the upper limit $\dim(W)$ even when $W$ is infinite-dimensional, where this is not a finite integer.}
...
Suppose $V$ is a Hilbert space and the subspace $W$ is spanned by $\vecnot{w}_1, \ldots, \vecnot{w}_n \in V$.
...
This follows from noting that $M\vecnot{v} = \vecnot{0}$ for $\vecnot{v}\neq\vecnot{0}$ implies that $\vecnot{v}^H M \vecnot{v} = 0$ and contradicts the definition of positive definite.
...
G \vecnot{s} = \vecnot{t}.
\end{equation*}
Since $J$ is a convex quadratic and $G$ is positive semidefinite, every solution of $G\vecnot s=\vecnot t$ is a global minimizer.
If $G$ is positive definite, then the minimizer is unique.
%we compute the mixed Wirtinger derivative $\partial^2J/(\partial\overline{\vecnot s}\,\partial\vecnot s^T)$
%\begin{equation*}
...
\end{equation*}
for $i = 1, \ldots, n$.
Define the error component $e_i$ by $e_i = y_i - a x_i - b$, then
\begin{equation*}
...
\end{equation*}
This equation has a form analogous to the matrix representation of a least-squares problem.
Consider the goal of minimizing $\left\| \vecnot{e} \right\|^2$.
If the two columns of $A$ are linearly independent, then the line that minimizes the sum of the squares of the vertical distances between the data points and the line is then given by
\begin{equation*}
\vecnot{s} = \left( A^H A \right)^{-1} A^H \vecnot{y}.
\end{equation*}
Otherwise, the minimizing line is not unique and any solution of $A^H (A \vecnot{s}-\vecnot{y})=\vecnot{0}$ minimizes the sum of squares.


\subsection{Linear Minimum Mean-Squared Error Estimation}

Let $Y,X_1,\ldots,X_n$ be zero-mean random variables with finite second moment.
...
Consider a sequence of zero-mean random variables $\left\{ X[t] \right\}$ its FIR filter output
\begin{equation*}
\begin{split}
...
\end{split}
\end{equation*}
The goal is to design this filter in such a way that its output is as close as possible to a desired sequence of zero-mean random variables $\left\{ Z[t] \right\}$.
...
If the zero-mean processes $\{X[t]\}$ and $\{Z[t]\}$ are jointly wide-sense stationary (i.e., their autocorrelation and cross-correlation functions exist and are shift invariant), then we have
\begin{equation*}
...
Using the vector $\vecnot{z} = \begin{bmatrix} z[K] & z[K+1] & \cdots & z[N] \end{bmatrix}^T$ and the matrix
...
and
\[ \left[ A^H \Lambda \vecnot{z} \right]_i = \sum_{t=K}^N \lambda^{N-t} z[t] \overline{x}[t-i+1] \approx \frac{R_{zx}(i-1)}{1-\lambda}. \]
...
Then, $G_{N+1} = \lambda G_N + \vecnot{u}^H \vecnot{u}$ and $\vecnot{b}_{N+1} = \lambda \vecnot{b}_N + z[N+1] \vecnot{u}^H$, where
\begin{equation}
...
A diagram for a minimum-norm solution problem in $\mathbb{R}^3$ appears in Figure~\ref{fig:AR-Dual1}.
...
Using \eqref{equation:DualApproximation}, one gathers that $\vecnot{y} = A^H A \vecnot{s} = M M^H \vecnot{s}$.
...
\begin{definition}
For a vector space $V$, a subset $A \subseteq V$ is called a \defn{vector space}{convex set} if, for all $\vecnot{a}_1,\vecnot{a}_2 \in A$ and $\lambda\in(0,1)$, we have $\lambda \vecnot{a}_1 + (1-\lambda) \vecnot{a}_2 \in A$.
Given a norm on $V$ and its induced topology, we say $A$ is \textbf{strictly convex} if $\lambda \vecnot{a}_1 + (1-\lambda) \vecnot{a}_2 \in \Interior{A}$ for $\lambda\in(0,1)$ when $\vecnot{a}_1,\vecnot{a}_2 \in \Closure{A}$ are distinct points.
\end{definition}
...
\begin{definition}
Let $V$ be a Hilbert space and $A\subseteq V$ be a nonempty closed convex set. The \textbf{orthogonal projection} of $\vecnot{v} \in V$ onto $A$ is defined by $P_A \colon V \to A$ with
\[
...
\begin{theorem}
\label{theorem:HilbertProjectionTheorem} For Hilbert space $V$, the orthogonal projection of $\vecnot v \in V$ onto a nonempty closed convex set $A\subseteq V$ exists and is unique.
\end{theorem}
\begin{proof}
...
\end{align*}
Thus, $\Real \tinner{ \vecnot{v}-\vecnot{u}^* \,}{\, \vecnot{u}-\vecnot{u}^* } \leq \frac{\alpha}{2} \| \vecnot{u} - \vecnot{u}^* \|^2$.
...
\begin{corollary}
\label{corollary:convex_projection_nonexpansive}
Let $A$ be a nonempty closed convex subset of a Hilbert space $V$.
Then, for all $\vecnot{v},\vecnot{w}\in V$, the projection map $P_A:V\to A$ satisfies
\[
\left\|P_A(\vecnot{v})-P_A(\vecnot{w})\right\|
\leq
\left\|\vecnot{v}-\vecnot{w}\right\|
\]
and $P_A$ is Lipschitz continuous.
\end{corollary}

\begin{proof}
Let $\vecnot u=P_A(\vecnot v)$ and $\vecnot x=P_A(\vecnot w)$.
By Theorem~\ref{theorem:convex_proj_lt0}, the projection of $\vecnot{v}$ gives
$\Real\tinner{\vecnot v-\vecnot u}{\vecnot x-\vecnot u}\leq 0$.
Similarly, the projection of $\vecnot w$ implies that $ \Real\tinner{\vecnot w-\vecnot x}{\vecnot u-\vecnot x}\leq 0$.
Since
\[
\vecnot v-\vecnot w
=
...
it follows that
\begin{align*}
\Real\tinner{\vecnot v-\vecnot w}{\vecnot u-\vecnot x}
&=
\Real\tinner{\vecnot v-\vecnot u}{\vecnot u-\vecnot x}
+\|\vecnot u-\vecnot x\|^2
+\Real\tinner{\vecnot x-\vecnot w}{\vecnot u-\vecnot x} \\
&\geq \|\vecnot u-\vecnot x\|^2.
\end{align*}
...
\Real\tinner{\vecnot v-\vecnot w}{\vecnot u-\vecnot x}
\leq
\|\vecnot v-\vecnot w\| \, \|\vecnot u-\vecnot x\|.
\]
If $\vecnot u \neq \vecnot x$, dividing by $\|\vecnot u-\vecnot x\|$ gives the stated result.
If not, it holds trivially.
\end{proof}
...
Mathematically, this means that, for all $\vecnot v,\vecnot v_{0}\in V$, the projection onto $A$ satisfies
\begin{align*}
...
A \defn{vector space}{hyperplane} is a closed subspace $U\subseteq V$ for which there is a non-zero $\vecnot{w}\in V$ such that $U=\{\vecnot{v}\in V:\inner{\vecnot{v}}{\vecnot{w}}=0\}=\Span(\vecnot{w})^{\bot}$.
...
\begin{proof}
Consider the convex set $A = \{B \vecnot{s} \, | \, \vecnot{s} \geq \vecnot{0} \}$, where $\vecnot{s} \geq \vecnot{0}$ denotes $s_i \geq 0$ for $i=1,\ldots,n$.
It is closed because it is a polyhedron equal to the intersection of finitely many closed half-spaces.
...
Now consider the smallest distance between two disjoint closed convex sets $A,B\subseteq V$.  Several things can go wrong.

Now, consider the smallest distance between two disjoint closed convex sets $A,B\subseteq V$.
In this case, a unique solution may exist but a some things can go wrong.
If the sets are not strictly convex (consider two squares, for example), multiple pairs of points may attain the minimum distance.
Even if both sets are strictly convex, a minimizing sequence may escape to infinity, so the infimum need not be attained.
For example, consider the strictly convex hyperbolic sets $A=\left\{ (x,y)|x^{2}-y^{2}\geq1,x>0\right\} $ and $B=\left\{ (x,y)|y^{2}-x^{2}\geq1,y\geq0\right\} $. These two sets share the line $x=y>0$ as an asymptote, so their infimal distance is 0.
...
The continuity of $P_A$ given by Corollary~\ref{corollary:convex_projection_nonexpansive} shows that $u_{n_{j}} \to \vecnot{u}^* = P_A (\vecnot{v}^*)$ and this implies $\| \vecnot{u}^* - \vecnot{v}^* \| = d$.
...
\]
Also, each component of the $(\vecnot{u},\vecnot{v})$ points in $M$ must lie on the boundary of its set because otherwise one could reduce the smallest distance by moving one point along the minimum distance line towards the boundary.
Now, suppose that (i) $A$ is strictly convex and (ii) $M$ contains more than one pair of minimizers.
Then, condition (ii) implies that there must be two boundary points $\vecnot u_{1},\vecnot u_{2}\in\partial A$ such that $\alpha\vecnot u_{1}+(1-\alpha)\vecnot u_{2}\in\partial A$ for $\alpha\in[0,1]$.
If $\vecnot{u}_1=\vecnot{u}_2$, uniqueness of the projection of this point onto $B$ gives $\vecnot{v}_1=\vecnot{v}_2$.
Thus two distinct minimizing pairs must have $\vecnot{u}_1\ne\vecnot{u}_2$.
But this contradicts condition (i) and shows that, if $A$ is strictly convex, then there is at most one pair $(\vecnot u^{*},\vecnot v^{*})\in M$ of minimizing points.
\end{proof}
