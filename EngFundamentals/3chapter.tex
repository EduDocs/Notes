...
In this example, complete vector space $V=\mathbb{R}^3$ is endowed with the $\ell_1$-norm  $\| \cdot \|_1$; the subset $W$ corresponds to the $xz$-plane; the vector $\vecnot{v}$ is the point in blue; and its best approximation $\vecnot{w}$ is shown in red.
...
%2 \Real \inner{ \vecnot{v} - \vecnot{w} }{ \vecnot{w}'' }
%+ \left\| \vecnot{w}'' \right\|^2 \geq 0
%\end{equation}
%for every $\vecnot{w}'' \in W$.
%If $\vecnot{w}'$ is in $W$ and $\vecnot{w}' \neq \vecnot{w}$, then we may take
%\begin{equation*}
%\vecnot{w}'' = - \frac{ \inner{ \vecnot{v} - \vecnot{w} }{ \vecnot{w} - \vecnot{w}' } }{ \left\| \vecnot{w} - \vecnot{w}' \right\|^2 } \left( \vecnot{w} - \vecnot{w}' \right).
%\end{equation*}
...
%&= 2 \Real \inner{ \vecnot{v} - \vecnot{w} }{ - \frac{ \inner{ \vecnot{v} - \vecnot{w} }{ \vecnot{w} - \vecnot{w}' } }{ \left\| \vecnot{w} - \vecnot{w}' \right\|^2 } \left( \vecnot{w} - \vecnot{w}' \right) } \\
%&\quad + \left\| - \frac{ \inner{ \vecnot{v} - \vecnot{w} }{ \vecnot{w} - \vecnot{w}' } }{ \left\| \vecnot{w} - \vecnot{w}' \right\|^2 } \left( \vecnot{w} - \vecnot{w}' \right) \right\|^2 \\
%&= - 2 \Real \frac{ \inner{ \vecnot{w} - \vecnot{w}' }{ \vecnot{v} - \vecnot{w} } }{ \left\| \vecnot{w} - \vecnot{w}' \right\|^2 }
%\inner{ \vecnot{v} - \vecnot{w} }{ \vecnot{w} - \vecnot{w}' }
%+ \frac{\left| \inner{ \vecnot{v} - \vecnot{w} }{ \vecnot{w} - \vecnot{w}' } \right|^2}{ \left\| \vecnot{w} - \vecnot{w}' \right\|^2 } \\
%&= - 2 \frac{ \left| \inner{ \vecnot{v} - \vecnot{w} }{ \vecnot{w} - \vecnot{w}' } \right|^2 }{ \left\| \vecnot{w} - \vecnot{w}' \right\|^2 }
%+ \frac{ \left| \inner{ \vecnot{v} - \vecnot{w} }{ \vecnot{w} - \vecnot{w}' } \right|^2 }{ \left\| \vecnot{w} - \vecnot{w}' \right\|^2 }
%= - \frac{ \left| \inner{ \vecnot{v} - \vecnot{w} }{ \vecnot{w} - \vecnot{w}' } \right|^2 }{ \left\| \vecnot{w} - \vecnot{w}' \right\|^2 } .
%\end{split}
%\end{equation}
%This inequality holds if and only if $\inner{ \vecnot{v} - \vecnot{w} }{ \vecnot{w} - \vecnot{w}' } = 0$.
...
Let $V$ be a Hilbert space and $\vecnot{w}_1 ,\vecnot{w}_2 , \ldots, \vecnot{w}_n$ be a set of linearly independent vectors in $V$.
The \defn{inner-product space}{dual approximation} problem is to find the vector $\vecnot{w}\in V$ of minimum-norm that satisfies $\inner{ \vecnot{w} }{ \vecnot{w}_i } = c_i$ for $i=1,\ldots,n$.
...
Let $A \in \ComplexNumbers^{m\times n}$ with $m<n$ be the matrix representation of an underdetermined system of linearly independent equations and $\vecnot{v} \in \ComplexNumbers^m$ be any column vector.
