...
\section{Computing Approximations in Hilbert Space}
...
Yet, there is a very close connection between Wiener filter and natural data driven approaches.
...
It is assumed the transmitted sequence ihas been convolved with an unknown LTI system.
...
A suitable cost funciton for this goal is
...
The \defn{inner-product space}{dual approximation} problem is to find the vector $\vecnot{w}\in V$ of minimum-norm that satisfes $\left\langle \vecnot{w} | \vecnot{w}_i \right\rangle = c_i$ for $i=1,\ldots,n$.
