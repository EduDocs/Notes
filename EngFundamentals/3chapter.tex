...
This vector is defined uniquely by
\[ \vecnot{w} = \sum_{i=1}^n s_i \vecnot{w}_i, \]
where $s_1,s_2,\ldots,s_n$ can be found by solving \eqref{eqn:DualNormalEquations} with the assumption $\left\langle \vecnot{v} | \vecnot{w}_i \right\rangle = c_i$.
\end{theorem}
\begin{proof}
...
This is because, for all $\vecnot{x} \in W^{\perp}$, we have $\left\langle \vecnot{x} | \vecnot{w}_i \right\rangle = 0$ and $\left\langle \vecnot{v} - \vecnot{x} | \vecnot{w}_i \right\rangle = c_i$.
...
With this knowledge, we let $\vecnot{w} = \vecnot{v}-\vecnot{x}^*$ and use~\eqref{eqn:DualNormalEquations} to solve for $s_1,\ldots,s_n$ because we know the RHS satisfies $\left\langle \vecnot{v} | \vecnot{w}_i \right\rangle = c_i$.
...
If the rows of $A$ are linearly independent, then the columns of $A^H$ are linearly independent and $(A A^H)^{-1}$ exists because the Gramian $A A^H$ is positive definite.
