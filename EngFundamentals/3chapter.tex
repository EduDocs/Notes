...
\end{split}
\end{equation}
% For the converse, we need to show that best approx implies orthogonality
% Thus, we assume best approx and suppose that some vector w'' is not orthogonal to the error (wolog ||w''||=1)
% Projecting the error onto w'' shows that ||v-w+w''|| < ||v-w|| and gives a contradiction
